...
describe('models display', () => {
  ...
  it('shows sustained-work capability labels for Kimi', () => {
    const display: ModelsDisplay = {
      models: [
        {
          id: 'kimi-code',
          tier: 'cloud',
          alias: 'kimi',
          capabilityLabels: ['dogfood-proven', 'sustained-work capable'],
        },
      ],
      routerUrl: 'http://localhost:8009',
      routerReachable: false,
    ...
    const out = formatModelsDisplay(display)
    expect(out).toContain('kimi-code (alias: kimi)')
    expect(out).toContain('dogfood-proven; sustained-work capable')
  })

  it('shows context capability labels separately from sustained-work labels', () => {
    const display: ModelsDisplay = {
      models: [
        {
          id: 'gpt-4.1',
          tier: 'cloud',
          contextLabels: ['1M ctx', 'ctx documented'],
        },
      ],
      routerUrl: 'http://localhost:8009',
      routerReachable: false,
    ...
    expect(out).toContain('1M ctx; ctx documented')
  })
