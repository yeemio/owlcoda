...
const ERROR_COLOR = themeColor('error')
...
describe('renderInline', () => {
  ...
  it('0.13.98: underscore italic is intentionally DISABLED (use *italic* instead)', () => {
    // _word_ is NEVER italic in OwlCoda's renderer — too many false-positives
    // on snake_case identifiers, env names, and architecture-diagram labels
    // (`_admission_*`, `cache_pre_gate`, `_controls`, etc.). Asterisk italic
    // remains active. See markdown.ts:renderInline for full rationale.
    expect(renderInline('this is _italic_ text')).toBe('this is _italic_ text')
    expect(renderInline('this is *italic* text')).toBe(`this is ${ITALIC}italic${RESET} text`)
  })
  ...
  it('renders bracketed status badges with semantic colors', () => {
    const result = renderInline('[OK] [PASS] [WARN] [FAIL] [ERROR]')
    expect(stripAnsi(result)).toBe('[OK] [PASS] [WARN] [FAIL] [ERROR]')
    expect(result).toContain(SUCCESS_COLOR)
    expect(result).toContain(WARNING_COLOR)
    expect(result).toContain(ERROR_COLOR)
  })

  it('renders compact status metrics without changing visible text', () => {
    const result = renderInline('PASS:14WARN:1FAIL:0')
    expect(stripAnsi(result)).toBe('PASS:14WARN:1FAIL:0')
    expect(result).toContain(SUCCESS_COLOR)
    expect(result).toContain(WARNING_COLOR)
    expect(result).toContain(ERROR_COLOR)
  })

  // Regression: `PASS:14OKAY` would emit `PASS:14` as a metric because the
  // bare `OK` substring in `OKAY` satisfied the lookahead alternation.
  // Whole-chain matching with bounded lookbehind/lookahead now requires the
  // run to be a real `LABEL:N(LABEL:N)*` chain bounded by non-word chars.
  it('does not color status:N when followed by an unrelated word starting with a label', () => {
    const result = renderInline('PASS:14OKAY')
    expect(stripAnsi(result)).toBe('PASS:14OKAY')
    expect(result).not.toContain(SUCCESS_COLOR)
    expect(result).not.toContain(WARNING_COLOR)
    expect(result).not.toContain(ERROR_COLOR)
  })

  // Regression: digit-prefixed labels (e.g., `HTTP200OK:1`, `A1WARN:2`)
  // would be colored because the previous prev-char check only excluded
  // letters/underscore, not digits. The chain regex's negative lookbehind
  // `(?<![A-Za-z0-9_])` now blocks both letter and digit prefixes.
  it('does not color label:N preceded by a digit (HTTP200OK:1)', () => {
    const result = renderInline('HTTP200OK:1')
    expect(stripAnsi(result)).toBe('HTTP200OK:1')
    expect(result).not.toContain(SUCCESS_COLOR)
  })

  it('does not color label:N preceded by an alphanumeric identifier (A1WARN:2)', () => {
    const result = renderInline('A1WARN:2')
    expect(stripAnsi(result)).toBe('A1WARN:2')
    expect(result).not.toContain(WARNING_COLOR)
  })

  it('does not render lowercase bracketed words as status badges', () => {
    const result = renderInline('press [ok] to continue, or [fail] to abort')
    expect(stripAnsi(result)).toBe('press [ok] to continue, or [fail] to abort')
    expect(result).not.toContain(SUCCESS_COLOR)
    expect(result).not.toContain(ERROR_COLOR)
  })

  it('keeps status-like text inside inline code as code', () => {
    const result = renderInline('literal `[FAIL]`')
    expect(stripAnsi(result)).toBe('literal [FAIL]')
    expect(result).toContain(INLINE_CODE_FG)
    expect(result).not.toContain(ERROR_COLOR)
  })
  ...
  it('0.13.98: even flanked _word_ stays literal (only *word* / **word** are emphasis)', () => {
    // Pre-0.13.98 we kept `_word_` italic behind a flanking-boundary regex
    // hoping to thread the needle. It still false-fires on `_admission_*`
    // (the `*` is a non-word boundary). The clean fix is to drop underscore
    // italic entirely — see markdown.ts:renderInline rationale block.
    expect(renderInline('say _hello_ world')).toBe('say _hello_ world')
    expect(renderInline('_start_ of line')).toBe('_start_ of line')
    expect(renderInline('end with _close_')).toBe('end with _close_')
    expect(renderInline('(_inside parens_)')).toBe('(_inside parens_)')
  })
  ...
  it('0.13.98: identifiers stay verbatim even when adjacent to emphasis-like spans', () => {
    // Real transcript pattern: explanation text mixes asterisk emphasis
    // with snake_case identifiers. The asterisk emphasis still works;
    // underscore spans stay literal so identifiers survive intact.
    const out = stripAnsi(renderInline(
      'The *helper* calls foo_bar_baz then returns *done*.',
    ))
    expect(out).toBe('The helper calls foo_bar_baz then returns done.')

    // Underscore-flanked spans no longer render emphasis — neither side wins.
    const literal = stripAnsi(renderInline(
      'The _helper_ calls foo_bar_baz then returns _done_.',
    ))
    expect(literal).toBe('The _helper_ calls foo_bar_baz then returns _done_.')
  })
})
...
describe('renderMarkdown', () => {
  ...
  it('renders report-style key/value lines while leaving prose labels plain', () => {
    const result = renderMarkdown('model: kimi-2.6\nprovider: kimi\nnote: leave plain')
    // kvLine and paragraph are distinct BlockKinds, so R6 inserts a blank
    // between the kv block (model + provider) and the paragraph (note line).
    // Both full-pass and streaming paths agree on this — see A13-A15 in the
    // path-equivalence fixtures.
    expect(stripAnsi(result)).toBe('model: kimi-2.6\nprovider: kimi\n\nnote: leave plain')
    expect(result).toContain(`${DIM}model:${RESET} kimi-2.6`)
    expect(result).toContain(`${DIM}provider:${RESET} kimi`)
    expect(result).not.toContain(`${DIM}note:${RESET}`)
  })

  // Regression: allowlisted single-word keys (cost, path, status, code, …)
  // are common English words. Without the prose-value guard, a sentence
  // beginning with one would be styled as a structured kv row, which is
  // misleading and visually wrong. The guard rejects values that start
  // with a lowercase word or digit run followed by 3+ lowercase letters,
  // while preserving identifier/number+unit/CJK/proper-noun values.
  it('leaves prose-shaped values as plain paragraphs even with allowlisted keys', () => {
    const lines = [
      'Status: in progress',
      'Cost: 100 total',
      'Path: through the woods',
      'Error: a deeper question',
    ]
    for (const line of lines) {
      const result = renderMarkdown(line)
      expect(stripAnsi(result)).toBe(line)
      expect(result).not.toContain(`${DIM}Status:${RESET}`)
      expect(result).not.toContain(`${DIM}Cost:${RESET}`)
      expect(result).not.toContain(`${DIM}Path:${RESET}`)
      expect(result).not.toContain(`${DIM}Error:${RESET}`)
    }
  })

  it('still renders compact data values (numbers + units, CJK, parenthetical)', () => {
    const result = renderMarkdown(
      'duration_ms: 519342\nlatency: 108 ms\nerror_kind: (MES404, 可解释)',
    )
    expect(result).toContain(`${DIM}duration_ms:${RESET} 519342`)
    expect(result).toContain(`${DIM}latency:${RESET} `)
    expect(stripAnsi(result)).toContain('latency: 108 ms')
    expect(result).toContain(`${DIM}error_kind:${RESET} `)
    expect(stripAnsi(result)).toContain('error_kind: (MES404, 可解释)')
  })

  // 3+ char common units (sec, bytes, …) bypass PROSE_VALUE_RE via the
  // VALUE_UNITS allowlist. Without the allowlist these would be rejected
  // as prose because `108 sec` matches `\d+\s+[a-z]{3,}`. Keys here are
  // chosen from KV_KEYS so the gate-1 (key shape) check passes.
  it('renders kvLine for number + 3+ char common units (sec, bytes, minutes)', () => {
    const lines = [
      'duration: 108 sec',
      'latency: 100 bytes',
      'runtime: 5 minutes',
      'total_tokens: 42 chars',
    ]
    for (const line of lines) {
      const result = renderMarkdown(line)
      const key = line.split(':')[0]!
      expect(result).toContain(`${DIM}${key}:${RESET} `)
    }
  })
...
describe('StreamingMarkdownRenderer word-wrap', () => {
  it('0.13.100: does NOT word-wrap moderately long lines (Latin threshold raised 120→800)', () => {
    // Pre-0.13.100 streaming wrapped Latin text > 120 chars, breaking
    // path-equivalence with full-pass (which doesn't wrap at all and
    // lets the terminal soft-wrap). New behavior: under 800 chars,
    // buffer stays unwrapped; terminal handles visual wrap.
    const renderer = new StreamingMarkdownRenderer()
    const words = Array.from({ length: 30 }, (_, i) => `word${i}`)
    const longText = words.join(' ')   // ~180 chars
    const out = renderer.push(longText)
    // Under threshold → buffered, no premature emission with embedded \n
    expect(out).toBe('')
  })

  it('0.13.100: wraps pathologically long Latin lines (> 800 chars)', () => {
    // Wrap still fires as a safety net for truly unbounded model output.
    const renderer = new StreamingMarkdownRenderer()
    const veryLong = Array.from({ length: 200 }, (_, i) => `word${i}`).join(' ') // ~1400 chars
    const out = renderer.push(veryLong)
...
describe('header / table boundary protection', () => {
  it('header line that contains | does not start a table', () => {
    // A heading like "## Tools | Comparison" must render as a header,
    // not start a table buffer that swallows the next pipe-line as a row.
    const r = new StreamingMarkdownRenderer()
    const out = r.push('## Tools | Comparison\n\n| col1 | col2 |\n|---|---|\n| a | b |\n') + r.flush()
    const plain = stripAnsi(out)
    // Header text should appear OUTSIDE table borders
    const tableStart = plain.indexOf('┌')
    const headerPos = plain.indexOf('Tools | Comparison')
    expect(headerPos).toBeGreaterThanOrEqual(0)
    expect(tableStart).toBeGreaterThan(headerPos)
  })

  it('header arriving while inTable=true flushes the buffered table cleanly', () => {
    // Pathological case: pipe-only lines start a table, then a header
    // arrives before the table's separator/data rows ever come. The
    // header must NOT be pushed into tableBuffer.
    const r = new StreamingMarkdownRenderer()
    const out = r.push('| col1 | col2 |\n# Header After\n') + r.flush()
    const plain = stripAnsi(out)
    expect(plain).toContain('Header After')
    // Plain pipe row falls back to literal rendering when no separator follows.
    expect(plain).toContain('col1')
  })

  it('table separator line `|------|------|` arriving without trailing \\n does not leak partial `|--`', () => {
    // tryStructuralBreak's pattern p6 (hrule: `(.{3,}?)(---+|___+|\*\*\*+)(?=[^\s*_-])`)
    // greedily matched the dashes inside `|------|------|`, breakAt landed
    // at position 3, and the renderer emitted `|--` as a partial line
    // followed by the rest of the separator as more partials. Visible to
    // users as stray `|--` / `----|` artifacts BEFORE the rendered table
    // border, exactly when a model emits a separator line in its own SSE
    // chunk (no trailing \n yet) and Phase 2 fires on the partial buffer.
    const r = new StreamingMarkdownRenderer()
    let out = ''
    // Step 1: header row arrives complete (with trailing \n)
    out += r.push('| a | b |\n')
    // Step 2: separator arrives WITHOUT trailing \n — Phase 2 runs on it
    out += r.push('|------|------|')
    const partialOutput = stripAnsi(out)
    // The renderer must NOT have emitted a fragment like `|--` standalone —
    // that would be Phase 2 hrule-pattern leakage on the table separator.
    expect(partialOutput).not.toMatch(/^\|--$/m)
    expect(partialOutput).not.toMatch(/^\|---+$/m)

    // Step 3: rest of table arrives, ensure final render is correct
    out += r.push('\n| 1 | 2 |\n')
    out += r.flush()
    const plain = stripAnsi(out)
    expect(plain).toContain('┌')
    expect(plain).toContain('└')
    // Cell content survives intact (nothing mangled by Phase 2 stealing dashes)
    expect(plain).toMatch(/│[^│]*1[^│]*│[^│]*2[^│]*│/)
  })
})
