...
describe('shouldShowNoResponseFallback', () => {
  ...
  it('returns false when stopReason is terminal_tool_failure', () => {
    // Terminal tool failures already have an explicit tool_result/error line.
    // Adding a generic no-response banner makes the operator chase the wrong layer.
    expect(shouldShowNoResponseFallback({
      finalText: '', stopReason: 'terminal_tool_failure', runtimeFailure: null, aborted: false,
    })).toBe(false)
  })

  it('returns false when stopReason is task_no_progress (deliberate no-progress hard-stop)', () => {
    // task_no_progress is a runtime synthesis decision exactly like hard_stop:
    // the guard intentionally stopped the loop and already surfaced its reason
    // (with actionable guidance) via onError. Stacking "(No response from
    // <model>)" on top reads as a transport/model failure and buries that
    // guidance — the exact low-info symptom seen in the mimo dogfood.
    expect(shouldShowNoResponseFallback({
      finalText: '', stopReason: 'task_no_progress', runtimeFailure: null, aborted: false,
    })).toBe(false)
  })

  it('returns false for deliberate runtime budget/identity stops', () => {
    for (const stopReason of ['narration_loop', 'max_iterations']) {
      expect(shouldShowNoResponseFallback({
        finalText: '', stopReason, runtimeFailure: null, aborted: false,
      })).toBe(false)
    }
  })
})
