...
describe('parseApiError', () => {
  ...
  it('preserves curated multi-line guidance instead of truncating it to 200 chars', () => {
    // Runtime-guard reasons (e.g. the no-progress hard-stop) ship multi-line
    // actionable guidance well past 200 chars. The single-line raw-blob cap
    // must not slice the "what to do" tail off.
    const reason =
      'task no-progress hard stop: 9 iterations elapsed under a write-requiring task contract with 0 touched paths.\n' +
      'Why: reads, searches and task-tracker calls do NOT count as deliverable progress.\n' +
      'What to do:\n' +
      '  • If the model is stuck: /retry, /model to switch, or re-send naming the exact file path(s).\n' +
      '  • If this task legitimately needs more turns: raise OWLCODA_TASK_NO_PROGRESS_ITER_LIMIT=20.'
    expect(reason.length).toBeGreaterThan(200)
    const out = parseApiError(reason)
    expect(out).toContain('OWLCODA_TASK_NO_PROGRESS_ITER_LIMIT') // the guidance tail survived
    expect(out).not.toContain('…') // not truncated mid-message
  })

  it('still caps a single-line raw blob so a huge error is not dumped whole', () => {
    const blob = `garbage ${'x'.repeat(500)}`
    const out = parseApiError(blob)
