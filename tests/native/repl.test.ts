...
describe('Slash Commands', () => {
  ...
  it('/fidelity shows read-only evidence-ledger telemetry', async () => {
    const os = await import('node:os')
    const fs = await import('node:fs')
    const path = await import('node:path')
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'owlcoda-fidelity-slash-'))
    ...
    try {
      ...
      fs.writeFileSync(path.join(telemetryDir, 'gate-events-2026-06-01.jsonl'), [
        JSON.stringify({
          ts: 100,
      ...
      const conv = makeConv()
      expect(await handleSlashCommand('/fidelity', conv, usage)).toBe(true)
      const output = logSpy.mock.calls.map(c => c[0]).join('\n')
      expect(output).toContain('Fidelity telemetry')
      expect(output).toContain('shadow-only, read-only')
      expect(output).toContain('Claims: 1 total, 0 matched, 1 unmatched')
      expect(output).toContain('Compaction facts: 1 total, 0 preserved, 1 dropped')
  ...
  it('/fidelity json prints bounded review samples with stable keys', async () => {
    const os = await import('node:os')
    const fs = await import('node:fs')
    const path = await import('node:path')
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'owlcoda-fidelity-json-'))
    const prevHome = process.env.OWLCODA_HOME
    process.env.OWLCODA_HOME = tmpDir
    try {
      const telemetryDir = path.join(tmpDir, 'telemetry')
      fs.mkdirSync(telemetryDir, { recursive: true })
      fs.writeFileSync(path.join(telemetryDir, 'gate-events-2026-06-01.jsonl'), JSON.stringify({
        ts: 100,
        kind: 'fidelity_claim_observed',
        conversationId: 'conv-fidelity-json',
        iteration: 1,
        lastToolSignatures: [],
        claimId: 'claim-1',
        anchorType: 'path',
      ...
      const conv = makeConv()
      expect(await handleSlashCommand('/fidelity json', conv, usage)).toBe(true)
      const output = logSpy.mock.calls.map(c => c[0]).join('\n')
      const parsed = JSON.parse(output)
      expect(parsed.totalEvents).toBe(1)
      expect(parsed.recent[0].target).toBe('src/native/nonexistent-ledger.ts')
      expect(parsed.recent[0].reviewKey).toMatch(/^[a-f0-9]{12}$/)
    } finally {
      if (prevHome === undefined) delete process.env.OWLCODA_HOME
      else process.env.OWLCODA_HOME = prevHome
      fs.rmSync(tmpDir, { recursive: true, force: true })
  ...
  it('/fidelity review prints a manual label packet', async () => {
    const os = await import('node:os')
    const fs = await import('node:fs')
    const path = await import('node:path')
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'owlcoda-fidelity-review-'))
    const prevHome = process.env.OWLCODA_HOME
    process.env.OWLCODA_HOME = tmpDir
    try {
      const telemetryDir = path.join(tmpDir, 'telemetry')
      ...
      fs.writeFileSync(path.join(telemetryDir, 'gate-events-2026-06-01.jsonl'), JSON.stringify({
        ts: 100,
        kind: 'fidelity_compaction_fact_observed',
        conversationId: 'conv-fidelity-review',
        iteration: 1,
        lastToolSignatures: [],
        factType: 'path',
      ...
      const conv = makeConv()
      expect(await handleSlashCommand('/fidelity review', conv, usage)).toBe(true)
      const output = logSpy.mock.calls.map(c => c[0]).join('\n')
      const parsed = JSON.parse(output)
      expect(parsed.schema).toBe('owlcoda.fidelity.review.v1')
      expect(parsed.samples[0].target).toBe('src/native/project-map.ts')
      expect(parsed.samples[0].reviewKey).toMatch(/^[a-f0-9]{12}$/)
      expect(parsed.samples[0].humanLabel).toBeNull()
      expect(parsed.samples[0].reviewerNotes).toBe('')
    } finally {
      if (prevHome === undefined) delete process.env.OWLCODA_HOME
      else process.env.OWLCODA_HOME = prevHome
      fs.rmSync(tmpDir, { recursive: true, force: true })
...
describe('slashCompleter', () => {
  ...
  it('matches /fi to /fidelity', () => {
    const [hits] = slashCompleter('/fi')
    expect(hits).toContain('/fidelity')
  })
...
describe('failed continuation submit handling', () => {
  ...
  it('blocks short continuation input under cheap retryable failures so the user re-confirms with /retry', () => {
    ...
    expect(isRetryEligibleContinuationFailure(httpRetryableFailure)).toBe(true)
    ...
    for (const text of ['继续', 'continue', 'resume', '请继续']) {
      expect(decideFailedContinuationSubmitAction({
        text,
        runtimeFailure: httpRetryableFailure,
        isRetryingFailedContinuation: false,
        failedContinuationAttemptCount: 1,
      })).toBe('guide_after_expensive_failure')
    }
  })

  it('guides instead of resending after expensive no-output failures', () => {
    ...
    for (const failure of [continuationFailure, toolContinuationFailure, requestPhaseFailure, timeoutFailure, emptyResponseFailure]) {
      expect(isExpensiveFailedContinuationFailure(failure)).toBe(true)
      // 0.13.96: only inputs that *exactly* match a recovery phrase trigger
      // the guide path. The previous list contained '为什么中断了？' /
      // '那你既然知道这么大，应该分批处理啊' / 'split into batches' — these
      // are real follow-up prompts that happened to mention a recovery
      // keyword (`中断`, `分批`, `split`); intercepting them blocked
      // multi-bullet questions and surprised the user. New rule only
      // classifies CONTINUATION_RETRY_PHRASES exact matches, polite
      // "请继续"-style anchored phrases, and pure punctuation as recovery.
      for (const text of ['继续', '继续写下去', 'continue', 'keep going']) {
        expect(isFailedRequestRecoveryInput(text)).toBe(true)
        expect(decideFailedContinuationSubmitAction({
          text,
      ...
      for (const text of ['为什么中断了？', '那你既然知道这么大，应该分批处理啊', 'split into batches']) {
        expect(isFailedRequestRecoveryInput(text)).toBe(false)
      }
    }
  })

  it('0.13.100: expensive failure guidance does NOT contain banned phrases', () => {
    // The wording contract from 0.13.97 (and reinforced here): never use
    // "before a usable response" or "OwlCoda will not resend the same
    // large context from a short recovery reply". formatExpensiveFailureGuidance
    // was missed in 0.13.97 (only buildLocalRecoveryGuidance was updated)
    // — user saw the old wording in 0.13.98 / 0.13.99 binaries. This
    // test guards the contract for ALL exit paths emitting guidance.
    const guidance = formatExpensiveFailureGuidance({
      kind: 'timeout',
      phase: 'request',
      message: 'kimi-code request failed: timeout after 120s',
      retryable: true,
    ...
    // Banned phrases — must NEVER appear, regardless of failure kind.
    expect(guidance).not.toContain('before a usable response')
    expect(guidance).not.toContain('will not resend the same large')
    expect(guidance).not.toContain('short recovery reply')
    expect(guidance).not.toContain('type "继续"')
    // Required guidance — must always offer at least one path forward.
    expect(guidance).toContain('/retry')
  })

  it('0.13.100: expensive failure guidance covers all expensive kinds', () => {
    // Each "expensive" kind (timeout / pre_first_token_stream_close /
    // empty_provider_response / stream_idle_timeout) must produce a
    // guidance string that is non-empty, mentions /retry, and avoids
    // the banned phrases.
    const kinds = ['timeout', 'pre_first_token_stream_close', 'empty_provider_response', 'stream_idle_timeout'] as const
  ...
  it('treats retryable empty-response provider failures as expensive recovery states', () => {
    ...
    // short recovery inputs are also blocked from implicit resend.
    const emptyResponseFailure = {
      kind: 'empty_provider_response' as const,
      phase: 'request' as const,
      message: 'No response from kimi-code: provider returned HTTP 200 but no content (stop_reason: end_turn)',
  ...
  it('covers phase=request (first-shot failure) with local recovery guidance', () => {
  ...
  it('extends recovery guidance to all retryable failure kinds, not just expensive ones', () => {
  ...
  it('does not schedule runtime auto-retry for timeout, pre-first-token failures, or queued user messages', () => {
    const timeoutFailure = {
      kind: 'timeout' as const,
      phase: 'request' as const,
      message: 'kimi-code request failed: timeout after 120s',
  ...
  it('does not schedule runtime auto-retry for empty provider responses (HTTP 200, end_turn, no content)', () => {
    ...
    // auto-continue gate suppress the loop. /retry stays explicit, while
    // short recovery inputs such as "继续" now get local guidance instead
    // of silently resending the same failed request.
    const emptyResponseFailure = {
      kind: 'empty_provider_response' as const,
      phase: 'request' as const,
      message: 'No response from kimi-code: provider returned HTTP 200 but no content (stop_reason: end_turn)',
  ...
  // failures such as HTTP 5xx.
  describe('same-kind 3-strike (0.13.63 C)', () => {
    const httpFailure = {
      kind: 'http_error' as const,
      phase: 'continuation' as const,
    ...
    it('schedules retry when no recent kinds tracked (back-compat)', () => {
      expect(shouldScheduleRuntimeAutoRetry({
        runtimeFailure: httpFailure,
        taskAborted: false,
        clearEpochUnchanged: true,
        currentRetryCount: 2,
        retryLimit: 8,
    ...
    it('schedules retry on first occurrence of kind (window has 0 prior)', () => {
      expect(shouldScheduleRuntimeAutoRetry({
        runtimeFailure: httpFailure,
        taskAborted: false,
        clearEpochUnchanged: true,
    ...
    it('schedules retry on second occurrence of kind (window has 1 prior)', () => {
      expect(shouldScheduleRuntimeAutoRetry({
        runtimeFailure: httpFailure,
        taskAborted: false,
        clearEpochUnchanged: true,
        ...
        retryLimit: 8,
        hasQueuedInput: false,
        recentRetryKinds: ['http_error'],
      })).toBe(true)
    })

    it('refuses retry on third occurrence of kind (window has 2 same-kind)', () => {
      expect(shouldScheduleRuntimeAutoRetry({
        runtimeFailure: httpFailure,
        taskAborted: false,
        clearEpochUnchanged: true,
        ...
        retryLimit: 8,
        hasQueuedInput: false,
        recentRetryKinds: ['http_error', 'http_error'],
      })).toBe(false)
    })

    it('still allows retry if last 2 kinds differ (mixed transient errors)', () => {
      expect(shouldScheduleRuntimeAutoRetry({
        runtimeFailure: httpFailure,
        taskAborted: false,
        clearEpochUnchanged: true,
        ...
        retryLimit: 8,
        hasQueuedInput: false,
        recentRetryKinds: ['connect_error', 'http_error'],
      })).toBe(true)
    })

    it('only the LAST 2 kinds count, not older history', () => {
      // Window contains [http_error, http_error, connect_error]; only the
      // last 2 ('http_error', 'connect_error') are checked. They differ →
      // allow retry.
      expect(shouldScheduleRuntimeAutoRetry({
        runtimeFailure: httpFailure,
        taskAborted: false,
        ...
        retryLimit: 8,
        hasQueuedInput: false,
        recentRetryKinds: ['http_error', 'http_error', 'connect_error'],
      })).toBe(true)
    })

    it('refuses when last 2 of mixed history are same as incoming', () => {
      expect(shouldScheduleRuntimeAutoRetry({
        runtimeFailure: httpFailure,
        taskAborted: false,
        clearEpochUnchanged: true,
        ...
        retryLimit: 8,
        hasQueuedInput: false,
        recentRetryKinds: ['connect_error', 'connect_error', 'http_error', 'http_error'],
      })).toBe(false)
    })
  })
  ...
  describe('conversationEndsAwaitingAssistant (resume continuation)', () => {
    it('returns false for an empty conversation', () => {
      expect(conversationEndsAwaitingAssistant([])).toBe(false)
    })

    it('returns false when the assistant spoke last (conversation at rest)', () => {
    ...
    it('returns true when the last turn is an unanswered user message', () => {
      expect(conversationEndsAwaitingAssistant([
        { role: 'user' },
        { role: 'assistant' },
        { role: 'user' },
    ...
    it('returns true for an interrupted tool loop ending in consecutive user turns', () => {
      // The real resume bug (conv-...smd2dt): assistant(tool_use) →
      // user(tool_result + text) → user(text), and no assistant turn ever
      // consumed the tool result. The session ends awaiting an assistant
      // response and must be offered a continuation, not left silent.
      expect(conversationEndsAwaitingAssistant([
        { role: 'assistant' },
        { role: 'user' },
        { role: 'user' },
  ...
  describe('shim coverage: every failure kind × input kind cell', () => {
    const failureKinds = [
      'timeout',
    ...
    const mkFailure = (kind: typeof failureKinds[number]) => ({
      kind,
      phase: 'request' as const,
      message: 'shim coverage test',
      retryable: true,
    })

    for (const kind of failureKinds) {
      it(`${kind} × /retry → retry_failed_continuation (force resubmit)`, () => {
        expect(decideFailedContinuationSubmitAction({
          text: '/retry',
          runtimeFailure: mkFailure(kind),
          isRetryingFailedContinuation: false,
          failedContinuationAttemptCount: 1,
        })).toBe('retry_failed_continuation')
      })

      it(`${kind} × /retry while already retrying → dedupe_retry_failed_continuation`, () => {
        expect(decideFailedContinuationSubmitAction({
          text: '/retry',
          runtimeFailure: mkFailure(kind),
          isRetryingFailedContinuation: true,
      ...
      it(`${kind} × /retry after 2+ failures → guide_after_repeated_failed_continuation`, () => {
        expect(decideFailedContinuationSubmitAction({
          text: '/retry',
          runtimeFailure: mkFailure(kind),
          isRetryingFailedContinuation: false,
      ...
      it(`${kind} × short recovery 继续 → guide_after_expensive_failure (block, no resend)`, () => {
        expect(decideFailedContinuationSubmitAction({
          text: '继续',
          runtimeFailure: mkFailure(kind),
          isRetryingFailedContinuation: false,
      ...
      it(`${kind} × long fresh content → submit_text (new turn, abandon failed)`, () => {
        expect(decideFailedContinuationSubmitAction({
          text: 'Now refactor the auth middleware to use the new session store and add tests for the migration path including rollback semantics',
          runtimeFailure: mkFailure(kind),
          isRetryingFailedContinuation: false,
          failedContinuationAttemptCount: 1,
        })).toBe('submit_text')
      ...
      it(`${kind} × other slash /clear → submit_text (passthrough; slash handler runs)`, () => {
        expect(decideFailedContinuationSubmitAction({
          text: '/clear',
          runtimeFailure: mkFailure(kind),
          isRetryingFailedContinuation: false,
    ...
    it('abort × /retry → retry_failed_continuation (explicit user resend)', () => {
      expect(decideFailedContinuationSubmitAction({
        text: '/retry',
        runtimeFailure: {
          kind: 'abort',
    ...
    it('abort × short recovery 继续 → submit_text (abort consumed, fresh ask)', () => {
      expect(decideFailedContinuationSubmitAction({
        text: '继续',
        runtimeFailure: {
          kind: 'abort',
    ...
    it('no failure × any text → submit_text (no recovery state)', () => {
      expect(decideFailedContinuationSubmitAction({
        text: 'fresh question with no failure context',
        runtimeFailure: null,
        isRetryingFailedContinuation: false,
        failedContinuationAttemptCount: 0,
      })).toBe('submit_text')
    })
  })
})
...
describe('/reset-circuits and /reset-budgets', () => {
  ...
  // 0.13.98: 5s default occasionally tripped under full-suite load —
  // /diff shells out to `git diff` which can spike past 5s when CI
  // disk / git resource contention is high. Sibling slash-command tests
  // in this file already use 10-15s for the same reason.
...
describe('REPL input helpers', () => {
  ...
  it('does not confirm exit from a duplicate interrupt in the same key burst', () => {
    expect(decideExitInterruptAction({ nowMs: 1000, lastExitAttemptMs: 0 })).toBe('arm')
    expect(decideExitInterruptAction({ nowMs: 1000, lastExitAttemptMs: 0, interruptsInInput: 2 })).toBe('arm')
    expect(decideExitInterruptAction({ nowMs: 1030, lastExitAttemptMs: 1000 })).toBe('ignore_duplicate')
    expect(decideExitInterruptAction({ nowMs: 1250, lastExitAttemptMs: 1000 })).toBe('confirm')
    expect(decideExitInterruptAction({ nowMs: 1400, lastExitAttemptMs: 1000, interruptsInInput: 2 })).toBe('ignore_duplicate')
    expect(decideExitInterruptAction({ nowMs: 2600, lastExitAttemptMs: 1000 })).toBe('arm')
  })

  it('counts both raw ETX and parsed Ctrl+C as interrupt input', () => {
    expect(countInterruptsInInput('c', { ctrl: true })).toBe(1)
    expect(countInterruptsInInput('C', { ctrl: true })).toBe(1)
    expect(countInterruptsInInput('\u0003\u0003', {})).toBe(2)
    expect(countInterruptsInInput('c', { ctrl: false })).toBe(0)
  })
...
describe('Additional slash commands', () => {
  ...
  it('/context uses the shared model context resolver for known long-context ids', async () => {
    const conv = makeConv('gpt-4.1')
    addUserMessage(conv, 'hello world')
    expect(await handleSlashCommand('/context', conv, usage)).toBe(true)
    const output = logSpy.mock.calls.flat().join(' ')
  ...
  it('/mode is disabled when OWLCODA_MODES explicitly opts out', async () => {
    const prev = process.env['OWLCODA_MODES']
    process.env['OWLCODA_MODES'] = '0'
    try {
      const conv = makeConv()
      expect(await handleSlashCommand('/mode plan', conv, usage)).toBe(true)
      expect(conv.options?.operatingModeState).toBeUndefined()
      const output = logSpy.mock.calls.flat().join(' ')
      expect(output).toContain('OWLCODA_MODES=0')
    } finally {
      if (prev === undefined) delete process.env['OWLCODA_MODES']
      else process.env['OWLCODA_MODES'] = prev
    }
  })

  it('/mode switches the current session by default', async () => {
    const prev = process.env['OWLCODA_MODES']
    delete process.env['OWLCODA_MODES']
    try {
      const conv = makeConv()
      expect(await handleSlashCommand('/mode plan', conv, usage)).toBe(true)
      expect(conv.options?.operatingModeState?.mode).toBe('plan')
      logSpy.mockClear()
      expect(await handleSlashCommand('/mode', conv, usage)).toBe(true)
      const output = logSpy.mock.calls.flat().join(' ')
  ...
  it('/project-map refresh stores a conversation snapshot without writing repo files', async () => {
    const fs = await import('node:fs')
    const os = await import('node:os')
    const path = await import('node:path')
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'owlcoda-project-map-slash-'))
    const prevCwd = process.cwd()
    const prevFlag = process.env['OWLCODA_PROJECT_MAP']
    ...
    try {
      ...
      fs.writeFileSync(path.join(tmpDir, 'AGENTS.md'), 'slash rules')

      const conv = makeConv()
      expect(await handleSlashCommand('/project-map refresh', conv, usage)).toBe(true)

      expect(conv.options?.projectMapSnapshot?.packageName).toBe('slash-fixture')
      expect(conv.options?.projectMapPromptSummary).toContain('<project_map>')
      expect(fs.existsSync(path.join(tmpDir, '.owlcoda-run', 'project-map.json'))).toBe(false)
      const output = logSpy.mock.calls.flat().join('\n')
      expect(output).toContain('Project map')
      expect(output).toContain('Enabled:   yes')
      expect(output).toContain('Package:   slash-fixture@1.0.0')
    } finally {
      process.chdir(prevCwd)
      if (prevFlag === undefined) delete process.env['OWLCODA_PROJECT_MAP']
      else process.env['OWLCODA_PROJECT_MAP'] = prevFlag
      fs.rmSync(tmpDir, { recursive: true, force: true })
    }
  })
...
describe('REPL interaction stabilization', () => {
  ...
  it('slash picker describes /fidelity as a telemetry surface', () => {
    const items = buildSlashPickerItems()
    const item = items.find((candidate) => candidate.value === '/fidelity')
    expect(item?.description).toMatch(/fidelity|telemetry|ledger/i)
  })
...
describe('selectVisibleTranscriptWindow', () => {
  ...
  it('live: tail-anchored newest item larger than budget shows completion tail', () => {
    const newestText = Array.from({ length: 12 }, (_, i) => `line ${i}`).join('\n')
    const items: TranscriptItem[] = [
      { id: 'prior', text: 'assistant text' },
      { id: 'newest', text: newestText, overflowAnchor: 'tail' },
    ]
    const result = selectVisibleTranscriptWindow(items, 80, 6, 0)

    expect(result.visible).toHaveLength(1)
    const visible = result.visible[0]!
    expect(visible.id).toMatch(/^newest::6-12$/)
    expect(visible.text).toBe('line 6\nline 7\nline 8\nline 9\nline 10\nline 11')
    expect(result.viewStartLine).toBeGreaterThan(0)
    expect(result.viewEndLine).toBe(result.totalLines)
  })

  it('splits long tail-anchored assistant transcript text before item-based scrollback', () => {
    const text = Array.from({ length: 16 }, (_, i) => `assistant line ${i}`).join('\n')
    const chunks = splitTranscriptAppendText(text, 'tail')

    expect(chunks).toHaveLength(16)
    expect(chunks[0]).toEqual({ text: 'assistant line 0', overflowAnchor: undefined })
    expect(chunks.at(-1)).toEqual({ text: 'assistant line 15', overflowAnchor: 'tail' })
  })

  it('keeps short tail-anchored transcript text as one item', () => {
    const text = Array.from({ length: 12 }, (_, i) => `line ${i}`).join('\n')
    expect(splitTranscriptAppendText(text, 'tail')).toEqual([{ text, overflowAnchor: 'tail' }])
  })

  it('splits tail-anchored text by display rows, not logical lines', () => {
    const text = Array.from({ length: 12 }, (_, i) => `logical line ${i} ` + 'x'.repeat(40)).join('\n')
    const chunks = splitTranscriptAppendText(text, 'tail', 24)

    expect(chunks.length).toBeGreaterThan(12)
    expect(chunks[0]!.text).toContain('logical line 0')
    expect(chunks.at(-1)!.overflowAnchor).toBe('tail')
  })

  it('splits a single extra-wide tail-anchored code line into scrollback-owned display rows', () => {
    const text = '```text\n' + 'COLUMN_'.repeat(80) + '\n```'
    const chunks = splitTranscriptAppendText(text, 'tail', 40)
...
//       entirely.
//
// Root cause: composeAssistantChunk buffers the trailing no-newline
// line in state.leftover. The runConversationTurn turn-end path must
// drain that leftover before stats/footer entries such as "(N iterations)"
// and usage are appended, otherwise the final answer tail appears after
// those footers and looks like a duplicated tail block. These tests
// exercise the full pipeline (composer + StreamingMarkdownRenderer)
// including the synthetic-\n drain used at turn end.
...
describe('composeAssistantChunk → StreamingMarkdownRenderer pipeline', () => {
  ...
  async function runPipelineWithToolBoundaries(events: Array<string | { tool: string }>): Promise<string> {
    const { StreamingMarkdownRenderer } = await import('../../src/native/markdown.js')
    const state = freshState()
    const renderer = new StreamingMarkdownRenderer()
    let output = ''
    ...
    for (const event of events) {
      if (typeof event === 'string') {
        const composed = composeAssistantChunk(event, state)
        if (composed) output += renderer.push(composed)
      } else {
        drainBoundary()
        output += `[tool:${event.tool}]\n`
      }
    }
    drainBoundary()
    return output
  ...
  it('turn-end leftover drain stays before iteration and usage footer entries', async () => {
    const answer = await runPipeline([
      'Body paragraph.\n',
      'Conclusion: final no-newline summary',
    ])
    const transcript = `${answer}(5 iterations)\n[10K in / 1K out]\n`

    const finalIndex = transcript.indexOf('final no-newline summary')
    const footerIndex = transcript.indexOf('(5 iterations)')
    expect(finalIndex).toBeGreaterThanOrEqual(0)
    expect(footerIndex).toBeGreaterThanOrEqual(0)
    expect(finalIndex).toBeLessThan(footerIndex)
  })

  it('tool boundaries drain no-newline narration before the next assistant chunk', async () => {
    const out = await runPipelineWithToolBoundaries([
      'Smoke passed, old file deleted.',
      { tool: 'read' },
      'Let me check the target vendor sections.',
      { tool: 'bash' },
      'Smoke already passed.',
    ])

    expect(out).toContain('Smoke passed, old file deleted.\n[tool:read]')
    expect(out).toContain('Let me check the target vendor sections.\n[tool:bash]')
    expect(out).not.toContain('deleted.Let me')
    expect(out).not.toContain('sections.Smoke')
  })
})
...
// ─── Task D-3: spinnerFrame not in ScrollableTranscriptProps ─────────────────
describe('ScrollableTranscriptProps (D-3)', () => {
  it('ScrollableTranscriptProps does NOT include spinnerFrame', () => {
    // Compile-level assertion: if spinnerFrame exists on the type, this fails.
    expectTypeOf<ScrollableTranscriptProps>().not.toHaveProperty('spinnerFrame')
  })
})

// ─── Task D-4: MAX_VISIBLE_ITEMS_CAP guard ───────────────────────────────────
describe('MAX_VISIBLE_ITEMS_CAP (D-4)', () => {
  function mk(n: number): { id: string; text: string }[] {
    return Array.from({ length: n }, (_, i) => ({ id: `id-${i}`, text: `item ${i}` }))
  }

  it('MAX_VISIBLE_ITEMS_CAP is 200', () => {
    expect(MAX_VISIBLE_ITEMS_CAP).toBe(200)
  })

  it('RECENT_VISIBLE_ITEMS (20) is well under MAX_VISIBLE_ITEMS_CAP', () => {
    // We cannot import RECENT_VISIBLE_ITEMS (it is file-local in ink-repl.tsx),
    // so we test via splitTranscriptForScrollback: for 500 items with window=20,
    // visible.length must be 20 which is <= 200.
    const items = mk(500)
    const { visible } = splitTranscriptForScrollback(items, 20)
    expect(visible.length).toBeLessThanOrEqual(MAX_VISIBLE_ITEMS_CAP)
  })

  it('splitTranscriptForScrollback visible.length never exceeds MAX_VISIBLE_ITEMS_CAP', () => {
    // With 500 items at window=20, visible is capped at 20 << 200
    const items = mk(500)
    const { visible } = splitTranscriptForScrollback(items, 20)
  ...
  it('visible.length === Math.min(items.length, window) for existing behavior (regression pin)', () => {
    // Pin the existing semantics: visible gets exactly min(n, window) items
