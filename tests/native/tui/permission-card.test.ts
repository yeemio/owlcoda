...
function expandCursorAdvance(emit: string): string {
  // CSI N C (cursor forward) → N spaces. Lets stripAnsi-then-split see
  // intra-row gaps as physical columns rather than collapsing to nothing.
  let s = emit.replace(/\x1b\[([0-9]*)C/g, (_match, n: string) => ' '.repeat(n ? Number(n) : 1))
  // CSI r;c H (cursor position, absolute) → \n separator. The in-place
  // repaint path advances rows via absolute CUP instead of LF, so without
  // this rewrite stripAnsi-and-split sees row content concatenated on a
  // single line.
  s = s.replace(/\x1b\[([0-9]+);([0-9]+)H/g, '\n')
  // CSI K (erase in line) → drop. The in-place path terminates each row
  // with EL; in the stripped/expanded view these are just row terminators
  // already implied by the CUP→\n rewrite above.
  s = s.replace(/\x1b\[K/g, '')
  return s
}
