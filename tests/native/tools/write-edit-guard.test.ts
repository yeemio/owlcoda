...
describe('write tool — fs-policy guard', () => {
  ...
  it('refuses empty path', async () => {
    ...
    expect(result.isError).toBe(true)
    // 0.13.59: schema-error path now intercepts before fs-policy with
    // a structured message naming the missing field. Either phrasing
    // is acceptable for the "refuse empty path" contract.
    expect(String(result.output)).toMatch(/non-empty string|missing required field.*`path`/)
  })
})

describe('write tool — tilde expansion (P0: literal `~/.ssh/x` was bypassing sensitive-path check)', () => {
  let fakeHome: string
  let prevHome: string | undefined

  beforeEach(() => {
    fakeHome = mkdtempSync(join(tmpdir(), 'owlcoda-tool-guard-home-'))
  ...
  it('refuses `~/.ssh/poc` and does NOT create a literal `~` directory in cwd', async () => {
    const tool = createWriteTool()
    const result = await tool.execute({ path: '~/.ssh/poc', content: 'pwn' } as any)
    expect(result.isError).toBe(true)
    expect(String(result.output)).toMatch(/SSH credentials/)
    expect(result.metadata?.['fsPolicyDenied']).toBe(true)

    // The pre-fix bug: a literal `~` directory would appear inside cwd
    // (the workspace) because resolve(workspaceRoot, '~/.ssh/poc')
    // produced `<workspaceRoot>/~/.ssh/poc`. Pin that this never happens.
    expect(existsSync(join(workspace, '~'))).toBe(false)
    const entries = readdirSync(workspace)
    expect(entries).not.toContain('~')

    // And of course the real ~/.ssh/poc must not exist either.
    expect(existsSync(join(fakeHome, '.ssh', 'poc'))).toBe(false)
  })
})
