...
describe('recordRequestMetrics', () => {
  ...
  it('accumulates per-model cache read/write tokens (defaulting absent to 0)', () => {
    recordRequestMetrics({ modelId: 'm', inputTokens: 100, outputTokens: 50, cacheReadTokens: 1024, cacheWriteTokens: 30, durationMs: 200, success: true })
    recordRequestMetrics({ modelId: 'm', inputTokens: 100, outputTokens: 50, cacheReadTokens: 512, durationMs: 200, success: true })
    recordRequestMetrics({ modelId: 'm', inputTokens: 100, outputTokens: 50, durationMs: 200, success: true }) // no cache fields → +0

    const m = getModelMetrics('m')!
    expect(m.totalCacheReadTokens).toBe(1536) // 1024 + 512 + 0
    expect(m.totalCacheWriteTokens).toBe(30)  // 30 + 0 + 0
  })
