...
describe('model recommendation', () => {
  ...
  it('keeps Kimi eligible for long-task-like code work', () => {
    const config = makeConfig([
      { id: 'kimi-code', backendModel: 'kimi-for-coding' },
      { id: 'minimax-m27', backendModel: 'MiniMax-M2.7-highspeed' },
    ])
    const rec = recommendModel(config, 'code')
    expect(rec.recommended.modelId).toBe('kimi-code')
    const kimi = [rec.recommended, ...rec.alternatives].find(score => score.modelId === 'kimi-code')
    expect(kimi?.reasons).not.toContain('not signed off for sustained work')
  })
