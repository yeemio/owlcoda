...
describe('healthzMatchesConfig', () => {
  ...
  it('requires config fingerprint when orphan daemon claim path asks for it', () => {
    const config = {
      port: 8019,
      routerUrl: 'http://localhost:11435/v1',
      host: '0.0.0.0',
      models: [{ id: 'fresh', backendModel: 'fresh-backend', aliases: [], default: true }],
    }
    const matching = {
      ...baseHealthz,
    ...
    const stale = {
      ...baseHealthz,
      configFingerprint: configIdentityFingerprint({
        ...config,
        models: [{ id: 'minimax-m27', backendModel: 'MiniMax-M2.7-highspeed', endpoint: 'https://api.minimax.io/anthropic' }],
      }),
    }
    expect(healthzMatchesConfig(matching, config, { requireConfigFingerprint: true })).toBe(true)
    expect(healthzMatchesConfig(stale, config, { requireConfigFingerprint: true })).toBe(false)
    expect(healthzMatchesConfig({ ...baseHealthz }, config, { requireConfigFingerprint: true })).toBe(false)
  })
})
