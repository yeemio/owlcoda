...
describe('skill store', () => {
  ...
  it('loads Claude-style SKILL.md-only packages from the learned skills dir', async () => {
    const content = `---
name: Guizang PPT Skill
description: Generate horizontal web PPT decks
when_to_use: when creating a magazine-style presentation
---

# Magazine Web PPT

## Workflow
Read references and generate one self-contained HTML deck.`
    const dir = join(tempDir, 'skills', 'guizang-ppt-skill')
    await mkdir(dir, { recursive: true })
    await writeFile(join(dir, 'SKILL.md'), content, 'utf-8')

    const loaded = await loadSkill('guizang-ppt-skill')
    expect(loaded).not.toBeNull()
    expect(loaded!.name).toBe('Guizang PPT Skill')
    expect(loaded!.description).toBe('Generate horizontal web PPT decks')
    expect(loaded!.sourceMarkdown).toBe(content)
    expect(await skillExists('guizang-ppt-skill')).toBe(true)
    expect(await getSkillCount()).toBe(1)

    const list = await listSkills()
    expect(list.map(s => s.id)).toEqual(['guizang-ppt-skill'])

    const learned = await loadLearnedSkills()
    expect(learned.map(s => s.id)).toEqual(['guizang-ppt-skill'])
  })
...
describe('skill export/import', () => {
  ...
  it('imports a Claude-style skill directory and preserves package assets', async () => {
    const sourceDir = join(tempDir, 'external', 'guizang-ppt-skill')
    await mkdir(join(sourceDir, 'references'), { recursive: true })
    await writeFile(join(sourceDir, 'SKILL.md'), `---
name: Guizang PPT Skill
description: Generate magazine-style HTML PPT
---

# Guizang PPT

## Use
Follow the bundled references.`, 'utf-8')
    await writeFile(join(sourceDir, 'references', 'layouts.md'), '# Layouts\n', 'utf-8')

    const result = await importSkillsFromPath(sourceDir)

    expect(result).toEqual({ imported: 1, skipped: 0, errors: [] })
    expect(existsSync(join(tempDir, 'skills', 'guizang-ppt-skill', 'references', 'layouts.md'))).toBe(true)

    const loaded = await loadSkill('guizang-ppt-skill')
    expect(loaded!.name).toBe('Guizang PPT Skill')
    expect(loaded!.sourceMarkdown).toContain('# Guizang PPT')
  })

  it('imports a Claude-style SKILL.md path', async () => {
    const sourceDir = join(tempDir, 'external', 'html-deck-skill')
    await mkdir(sourceDir, { recursive: true })
    await writeFile(join(sourceDir, 'SKILL.md'), `---
name: HTML Deck Skill
description: Build a single-file deck
---

# HTML Deck Skill`, 'utf-8')

    const result = await importSkillsFromPath(join(sourceDir, 'SKILL.md'))

    expect(result.imported).toBe(1)
    expect(await skillExists('html-deck-skill')).toBe(true)
  })
})
