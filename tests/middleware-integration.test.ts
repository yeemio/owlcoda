...
describe('retry middleware', () => {
  ...
  it('gives up after max retries on retryable error', async () => {
    ...
    await expect(
      withRetry(
        async () => { attempts++; throw new Error('fetch failed: ECONNRESET') },
  ...
  it('does NOT retry timeout-shaped transport errors', async () => {
    let attempts = 0
    await expect(
      withRetry(
        async () => { attempts++; throw new Error('fetch failed: timeout') },
        { maxRetries: 2, baseDelayMs: 1 },
      ),
