...
async function test1_nonStreaming(): Promise<boolean> {
...
async function test2_streaming(): Promise<boolean> {
...
async function test3_toolUse(): Promise<boolean> {
  console.log('\n=== Test 3: Tool use ===')
  try {
    const msg = await client.messages.create({
      model: 'default',
...
async function test4_multiTurn(): Promise<boolean> {
...
async function test5_models(): Promise<boolean> {
...
async function test6_errorHandling(): Promise<boolean> {
...
async function main() {
  ...
  if (failed > 0) {
    console.error(`SDK verification failed: ${failed}/${results.length} checks failed`)
    process.exit(1)
  }
}
