...
async function startFakeManagedDaemon(options: {
  port: number
  routerUrl: string
  version: string
  runtimeToken?: string
  ...
  const code = `
    ...
    const server = http.createServer((req, res) => {
      ...
      if (req.url === '/health') {
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ status: 'ok', version, uptime: 1, models: [] }));
        return;
      }
      res.writeHead(404);
      res.end('not found');
  ...
  if (child.pid === undefined) throw new Error('Failed to spawn fake daemon')
  child.unref()
  sleeperPids.add(child.pid)

  const start = Date.now()
  while (Date.now() - start <= 5000) {
    try {
      const healthz = await fetchHealthz(options.port).then(res => res.json()) as { pid?: number; version?: string }
      if (healthz.pid === child.pid && healthz.version === options.version) return child
    } catch {
      // retry until the child starts listening
    }
    await new Promise(resolve => setTimeout(resolve, 100))
  }
  throw new Error('Fake managed daemon did not become ready')
}
...
describe('CLI lifecycle integration', { timeout: CLI_SUBPROCESS_TEST_TIMEOUT_MS }, () => {
  ...
  it('restarts a stale matching daemon when no live REPL client is attached', async () => {
    const runtimeDir = makeRuntimeDir()
    const port = await getFreePort()
    const routerUrl = 'http://127.0.0.1:65526'
    const configPath = makeConfig(runtimeDir, port, routerUrl)
    const { pidPath, metaPath } = runtimePaths(runtimeDir)
    const fake = await startFakeManagedDaemon({ port, routerUrl, version: '0.13.36', runtimeToken: 'old-token' })
    if (fake.pid === undefined) throw new Error('Fake daemon missing pid')

    mkdirSync(runtimeDir, { recursive: true })
    writeFileSync(pidPath, `${fake.pid}\n`)
    ...
    const startResult = await runCli(['start', '--config', configPath], runtimeDir)
    expect(startResult.code).toBe(0)
    expect(startResult.stderr).toContain('no active live REPL clients')
    expect(startResult.stderr).toContain('started in background')

    const nextPid = Number(readFileSync(pidPath, 'utf-8').trim())
    expect(nextPid).not.toBe(fake.pid)
    sleeperPids.delete(fake.pid)
    const healthz = await fetchHealthz(port).then(res => res.json()) as { version?: string }
    expect(healthz.version).toBe(VERSION)

    await runCli(['stop'], runtimeDir)
  }, CLI_SUBPROCESS_TEST_TIMEOUT_MS)

  it('hard-blocks a stale matching daemon when a live REPL client is attached', async () => {
    const runtimeDir = makeRuntimeDir()
    const port = await getFreePort()
    const routerUrl = 'http://127.0.0.1:65525'
    const configPath = makeConfig(runtimeDir, port, routerUrl)
    const { pidPath, metaPath, leasePath } = runtimePaths(runtimeDir)
    const fake = await startFakeManagedDaemon({ port, routerUrl, version: '0.13.36', runtimeToken: 'old-token' })
    if (fake.pid === undefined) throw new Error('Fake daemon missing pid')
    ...
    expect(startResult.stderr).toContain('will not restart it automatically')
    expect(startResult.stderr).toContain('owlcoda stop --force && owlcoda')
    expect(startResult.stderr).toContain('Refusing to start with version-drifted daemon')
    expect(startResult.stderr).toContain('OWLCODA_ALLOW_VERSION_DRIFT=1')
    expect(Number(readFileSync(pidPath, 'utf-8').trim())).toBe(fake.pid)
    expect(isPidAlive(fake.pid)).toBe(true)
