...
describe('config-display module', () => {
  ...
  it('shows an explicit default with Kimi sustained-work labels', () => {
    const display: ConfigDisplay = {
      configPath: '/tmp/config.json',
      configExists: true,
      version: '1.0.3',
      ...
      models: [
        ...
        { id: 'minimax-m27', label: 'MiniMax M2.7', backendModel: 'MiniMax-M2.7-highspeed', tier: 'cloud', isDefault: false, aliases: ['minimax'], capabilityLabels: [] },
      ],
      launchMode: 'native',
      skillInjection: true,
      nativeToolCount: 42,
    ...
    const output = formatConfigDisplay(display)
    expect(output).not.toContain('configured default ignored')
    expect(output).toContain('sustained-work capable')
