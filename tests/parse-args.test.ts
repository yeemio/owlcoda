...
describe('parseArgs', () => {
  ...
  it('parses service command', () => {
    expect(parse('service').command).toBe('service')
  })

  it('routes "service install" subcommand into passthroughArgs', () => {
    const p = parse('service', 'install')
    expect(p.command).toBe('service')
    expect(p.passthroughArgs).toEqual(['install'])
  })

  it('does NOT let "service status" collapse to the status command', () => {
    // 'status' is its own command; the commandExplicit guard must keep it as a
    // subcommand of service, not override command to 'status'.
    const p = parse('service', 'status')
    expect(p.command).toBe('service')
    expect(p.passthroughArgs).toEqual(['status'])
  })

  it('keeps flags working after a service subcommand', () => {
    const p = parse('service', 'install', '--config', '/tmp/c.json')
    expect(p.command).toBe('service')
    expect(p.passthroughArgs).toEqual(['install'])
  ...
  it('parses --endpoint as canonical alias to --router (ADR endpoint-first)', () => {
    expect(parse('--endpoint', 'https://api.example.com/v1').routerUrl).toBe('https://api.example.com/v1')
  })
