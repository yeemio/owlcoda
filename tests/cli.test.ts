...
describe('parseArgs', () => {
  ...
  it('parses --allow-tool and --deny-tool lists', () => {
    const result = parseArgs(['node', 'cli.js', 'run', '--allow-tool', 'read,write', '--allow-tool', 'grep', '--deny-tool', 'bash,TaskCreate'])
    expect(result.allowTools).toEqual(['read', 'write', 'grep'])
    expect(result.denyTools).toEqual(['bash', 'TaskCreate'])
  })
  ...
  it('new flags default to undefined/false', () => {
    ...
    expect(result.allowTools).toEqual([])
    expect(result.denyTools).toEqual([])
...
describe('import safety', () => {
  ...
  it('rejects Node 18 before importing the REPL dependency path', async () => {
    const mod = await import('../src/cli.js')
    expect(mod.isSupportedNodeVersion('18.19.1')).toBe(false)
    expect(mod.isSupportedNodeVersion('20.18.1')).toBe(false)
    expect(mod.isSupportedNodeVersion('20.19.0')).toBe(true)
    expect(mod.isSupportedNodeVersion('22.12.0')).toBe(true)
    expect(mod.getUnsupportedNodeMessage('18.19.1')).toContain('Node.js >= 20.19.0')
  })
})
