...
describe('getDefaultConfiguredModel', () => {
  ...
  it('honors an explicit Kimi default set by the user', () => {
    const config = makeConfig([
      normalizeModel({ id: 'kimi-code', backendModel: 'kimi-for-coding', aliases: ['kimi'], default: true }),
      normalizeModel({ id: 'minimax-m27', backendModel: 'MiniMax-M2.7-highspeed', aliases: ['minimax'] }),
    ])
    expect(getDefaultConfiguredModel(config)!.id).toBe('kimi-code')
  })

  it('allows signed-off Kimi as an automatic default when it is first configured', () => {
    const config = makeConfig([
      normalizeModel({ id: 'kimi-code', backendModel: 'kimi-for-coding', aliases: ['kimi'] }),
      normalizeModel({ id: 'minimax-m27', backendModel: 'MiniMax-M2.7-highspeed', aliases: ['minimax'] }),
    ])
    expect(getDefaultConfiguredModel(config)!.id).toBe('kimi-code')
  })
  ...
  it('demotes legacy your-default-model placeholder when a real cloud model exists', () => {
    // Self-heal for users who init'd against an early build that wrote the
    // local placeholder, then added a cloud provider in Admin without
    // realizing the placeholder was still flagged default.
    const config = makeConfig([
      normalizeModel({ id: 'your-default-model', backendModel: 'your-default-model', default: true }),
      normalizeModel({
        id: 'minimax-m27',
    ...
    expect(getDefaultConfiguredModel(config)!.id).toBe('minimax-m27')
  })

  it('returns null when only the legacy placeholder is configured', () => {
    // When the placeholder is alone, doLaunch must treat that as "no usable
    // default" and route to admin onboarding rather than triggering preflight.
    const config = makeConfig([
      normalizeModel({ id: 'your-default-model', backendModel: 'your-default-model', default: true }),
    ])
...
describe('interactive chat model helpers', () => {
  ...
  it('honors an explicit Kimi default for preferred interactive mode', () => {
    const config = makeConfig([
      normalizeModel({ id: 'kimi-code', backendModel: 'kimi-for-coding', aliases: ['kimi'], default: true }),
      normalizeModel({ id: 'minimax-m27', backendModel: 'MiniMax-M2.7-highspeed', aliases: ['minimax'] }),
    ])
    expect(getPreferredInteractiveConfiguredModel(config)?.id).toBe('kimi-code')
  })

  it('allows signed-off Kimi for preferred interactive mode when it is first configured', () => {
    const config = makeConfig([
      normalizeModel({ id: 'kimi-code', backendModel: 'kimi-for-coding', aliases: ['kimi'] }),
      normalizeModel({ id: 'minimax-m27', backendModel: 'MiniMax-M2.7-highspeed', aliases: ['minimax'] }),
    ])
