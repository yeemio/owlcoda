...
describe('GET /v1/cost', () => {
  ...
  it('surfaces prefix-cache tokens per-model and in the session total', () => {
    // owlmlx / cloud prefix-cache hits flow through recordRequestMetrics +
    // addTokenUsage as cacheRead; /cost must surface them (previously only
    // /tokens did).
    recordRequestMetrics({ modelId: 'owlmlx-local', inputTokens: 200, outputTokens: 50, cacheReadTokens: 1024, cacheWriteTokens: 0, durationMs: 100, success: true })
    addTokenUsage(200, 50, 1024, 0)

    const { req, res } = createMockReqRes()
    handleCost(req, res)
    const body = JSON.parse(res._body)

    const entry = body.data.find((d: { model_id: string }) => d.model_id === 'owlmlx-local')
    expect(entry.cache_read_tokens).toBe(1024)
    expect(entry.cache_write_tokens).toBe(0)
    expect(body.session.total_cache_read_tokens).toBe(1024)
    expect(body.session.total_cache_write_tokens).toBe(0)
  })
