...
describe('messages.ts retry wiring', () => {
  ...
  it('wraps fetch with withRetry on both streaming and non-streaming paths', () => {
    // 0.13.100: streaming path's withRetry callback shape changed from
    // `() => fetch(...)` to `async () => { ...timer setup...; try { return
    // await fetch(...) } finally { clearTimeout(...) } }` because the
    // streaming fetch needed a manually-managed headers timeout (replacing
    // the body-killing AbortSignal.timeout). Check that withRetry IS the
    // call site driving each fetch, without prescribing the exact arrow
    // shape.
    const withRetryCount = (messagesSource.match(/withRetry\(/g) ?? []).length
    expect(withRetryCount).toBeGreaterThanOrEqual(2)
    // Each withRetry should bracket an actual fetch call.
    const fetchInRetryBody = (messagesSource.match(/withRetry\([\s\S]*?fetch\(/g) ?? []).length
    expect(fetchInRetryBody).toBeGreaterThanOrEqual(2)
  })
})
