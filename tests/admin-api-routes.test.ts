...
import { loadConfig, type ConfiguredModel } from '../src/config.js'
...
function createHarness(options: {
  discoveryModels?: Array<{ id: string, label?: string, backend?: string, baseUrl?: string }>
  localRuntimes?: Array<{
    templateId: string
    label: string
    endpoint: string
  ...
  endpointHealthCache?: EndpointHealthCache
} = {}) {
  ...
  return {
    ...
    deps: {
      ...
      discoverLocalRuntimes: async () => options.localRuntimes ?? [],
      endpointHealthCache: options.endpointHealthCache,
      auth,
    },
  }
}
...
describe('admin api routes', () => {
  ...
  it('returns schemaVersion on all read endpoints and redacts config keys', async () => {
    ...
    try {
      ...
      expect(snapshotBody.version).toBe(VERSION)
  ...
  it('returns local runtime discovery hints from GET /admin/api/local-runtimes', async () => {
    const harness = createHarness({
      localRuntimes: [
        {
          templateId: 'ollama',
    ...
    try {
      const res = await invokeAdminRoute(harness.deps, '/admin/api/local-runtimes', {
  ...
  it('returns endpoint health samples from GET /admin/api/endpoint-health', async () => {
    ...
    const harness = createHarness({ endpointHealthCache: cache })

    try {
      const res = await invokeAdminRoute(harness.deps, '/admin/api/endpoint-health', {
        headers: { authorization: 'Bearer test-admin-secret' },
      })
  ...
  it('records endpoint-health sample after a test-connection probe and surfaces it on the cache route', async () => {
    const cache = new EndpointHealthCache()
    const harness = createHarness({ endpointHealthCache: cache })

    try {
      const probeRes = await invokeAdminRoute(harness.deps, '/admin/api/test-connection', {
        method: 'POST',
        headers: {
      ...
      const healthRes = await invokeAdminRoute(harness.deps, '/admin/api/endpoint-health', {
        headers: { authorization: 'Bearer test-admin-secret' },
      })
  ...
  it('records endpoint-health sample after a saved model probe', async () => {
    const cache = new EndpointHealthCache()
    const harness = createHarness({ endpointHealthCache: cache })

    try {
      const probeRes = await invokeAdminRoute(harness.deps, '/admin/api/models/saved-cloud/test', {
        method: 'POST',
        headers: {
          authorization: 'Bearer test-admin-secret',
          'content-type': 'application/json',
