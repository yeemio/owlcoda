...
# ─── 1b. Unit tests ────────────────────────────────────
echo "▸ Unit tests"
# Decide pass/fail by vitest's exit code, not by grepping its output: the
# summary line is ANSI-colorized ("Tests <ESC>[..m 5810 passed"), which a
# whitespace regex can't match and would mis-flag an all-green run as failed.
TEST_OUT=$(npm test 2>&1) && TEST_RC=0 || TEST_RC=$?
if [ "$TEST_RC" -eq 0 ]; then
  TEST_COUNT=$(printf '%s' "$TEST_OUT" | grep -aoE '[0-9]+ passed' | tail -1 | grep -oE '[0-9]+')
...
if command -v mcp-server-filesystem &>/dev/null; then
  ...
  if echo "$MCP_OUT" | grep -q "mcp__filesystem"; then
    ...
    # The model didn't recite mcp__filesystem in free text. That's an
    # unreliable probe (depends on the model faithfully listing every tool by
    # exact name), not proof the MCP server failed to load — so skip, don't
    # fail the gate on it.
    skip "MCP tools visible" "model did not recite mcp__filesystem (unreliable free-text probe)"
  fi
else
