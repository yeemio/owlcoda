 ...
 * @param projectRoot — override project root (default: auto-detected via import.meta.url)
 */
export function getCuratedSkillsDir(projectRoot?: string): string {
  return join(projectRoot ?? getProjectRoot(), 'skills')
}
 ...
 * @param projectRoot — override project root (default: auto-detected via import.meta.url)
 */
export async function loadCuratedSkills(projectRoot?: string): Promise<SkillDocument[]> {
  const dir = getCuratedSkillsDir(projectRoot)
