...
interface InkPickerProps<T> {
  ...
  onCancel: () => void
  /**
   * Optional Enter-fallback: called with the raw query before onSelect. If it
   * returns true it handled the submit (e.g. a known slash command typed with
   * an argument that matched no item), and onSelect is skipped. Lets the slash
   * picker run `/mode normal` instead of dead-ending on "no matches".
   */
  onSubmitRaw?: (query: string) => boolean
}
...
export function InkPicker<T>({
  ...
  useInput((input, key) => {
    ...
    if (key.return) {
      // Enter-fallback first: lets a known command typed with an argument
      // (e.g. "mode normal") submit even when it fuzzy-matched no item.
      if (onSubmitRaw && onSubmitRaw(query)) return
