...
export interface ConfigToolDeps {
   ...
   * would normally fire auto-resolves to "deny" instead of waiting on
   * a human keypress. Mostly a QA aid: lets autonomous flows verify
   * the safety gate is actually firing (the tool result will be
   * "Tool execution denied by user." rather than silent execution).
   */
  autoDeny?: {
    get: () => boolean
    set: (v: boolean) => void
...
export function createConfigTool(deps: ConfigToolDeps = {}): NativeToolDef<ConfigInput> {
  const settings: Record<string, SettingDef> = {
    ...
    autoDeny: {
      get: () => deps.autoDeny ? deps.autoDeny.get() : false,
      ...(deps.autoDeny ? { set: (v: unknown) => deps.autoDeny!.set(Boolean(v)) } : {}),
      type: 'boolean',
    },
  }
