...
export interface ToolSearchDeps {
  /** Resolves a tool's authored description so honest stub-disclosure
   * copy reaches the LLM instead of the `Native ${name} tool` placeholder. */
  getToolDescription?: (name: string) => string | undefined
}

export function createToolSearchTool(deps: ToolSearchDeps = {}): NativeToolDef<ToolSearchInput> {
  return {
    name: 'ToolSearch',
    description:
    ...
    async execute(input: ToolSearchInput): Promise<ToolResult> {
      ...
      const lines = matches.map(name => {
        ...
        return `<function>{"description": ${JSON.stringify(description)}, "name": "${name}", "parameters": ${JSON.stringify(schema)}}</function>`
      })
