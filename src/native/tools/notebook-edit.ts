...
import type { NotebookEditInput, NativeToolDef, ToolExecutionContext, ToolResult } from './types.js'
...
export function createNotebookEditTool(): NativeToolDef<NotebookEditInput> {
  return {
    ...
    async execute(input: NotebookEditInput, context?: ToolExecutionContext): Promise<ToolResult> {
      ...
      const policy = checkWritePathAllowed(notebook_path, { externalScopes })
      if (!policy.allowed) {
        return {
