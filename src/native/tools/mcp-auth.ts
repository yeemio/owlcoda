...
export function createMcpAuthTool(): NativeToolDef<McpAuthInput> {
  return {
    name: 'McpAuth',
    description:
      'Write an entry into an in-memory auth Map marking the given server_name as authenticated=true ' +
      'and recording the auth_type and timestamp. The token / client_id / client_secret are NOT validated, ' +
      'NOT exchanged with any server, NOT used for an OAuth round-trip, and NOT attached to subsequent MCP calls — ' +
      'any input rubber-stamps the server as authenticated. The entry is also lost when the process exits. ' +
      'Use this only if the user is testing the auth-status API or stubbing out a flow. ' +
      'For real MCP authentication, configure the server\'s credentials in .mcp.json (env vars, headers, ' +
      'or whatever the MCP server expects) and let the MCP client handle the handshake.',
