 ...
 * falls back to a readline prompt on stdin/stdout for legacy CLI hosts.
 * Deterministic headless callers should always provide a callback (or
 * deny the tool through approval) so machine-readable runs never block
 * on stdin.
 */
...
export function createAskUserQuestionTool(): NativeToolDef<AskUserQuestionInput> {
  return {
    ...
    async execute(
      ...
      // Legacy fallback: the host did not provide a UI callback. This is
      // intentionally not used by deterministic headless JSON runs, which
      // install an auto-cancel callback and deny the tool before execution.
