...
/**
 * 0.13.71 execution_economics_v1 — evidence_ledger_v1 (glob arm).
 *
 * Repeat-glob nudge mirrors the grep arm: same WeakMap-by-task
 * pattern, key on (pattern, cwd, ignore[]). Glob is even more
 * deterministic than grep within a short task — file trees rarely
 * change mid-task in ways the model didn't itself trigger — so a
 * second identical glob is almost always wasted. Same advisory-only
 * stance: don't block, prepend a runtime nudge.
 */
interface GlobEntry {
  count: number
  lastResultMatchCount: number
}
const globLedgers = new WeakMap<TaskExecutionState, Map<string, GlobEntry>>()

function makeGlobKey(input: GlobInput): string {
...
export function recordGlobAndBuildNudge(
  taskState: TaskExecutionState | undefined,
  input: GlobInput,
  matchCount: number,
): NudgeResult {
  if (!taskState) return { prefix: null, count: 1 }
  let ledger = globLedgers.get(taskState)
  if (!ledger) {
    ledger = new Map()
    globLedgers.set(taskState, ledger)
  }
  const key = makeGlobKey(input)
  ...
  const cwdNote = input.cwd ?? '(default cwd)'
  if (existing.count === 2) {
    return {
      prefix:
        `[Runtime glob-repeat notice]\n` +
        `You already ran this glob earlier in this task: pattern=${JSON.stringify(input.pattern)}, cwd=${cwdNote}.\n` +
        `Previous run matched ${prevMatchCount} file${prevMatchCount === 1 ? '' : 's'}; this run matched ${matchCount}. ` +
        `Reuse the prior file list — re-running the same glob rarely produces new entries.`,
      count: existing.count,
    }
  }
  return {
    prefix:
      `[Runtime glob-repeat warning]\n` +
      `This is the ${existing.count}${ordinalSuffix(existing.count)} run of the same glob in this task: ` +
      `pattern=${JSON.stringify(input.pattern)}, cwd=${cwdNote}.\n` +
      `Use the prior file list, narrow the pattern, or switch to grep/read on a specific file.`,
    count: existing.count,
  }
}

/** Test-only: clear the glob ledger for a given task. */
export function __resetGlobLedgerForTask(taskState: TaskExecutionState): void {
  globLedgers.delete(taskState)
}
...
export function createGlobTool(): NativeToolDef<GlobInput> {
  return {
    ...
    async execute(input: GlobInput, context?: ToolExecutionContext): Promise<ToolResult> {
      ...
      const result = successResult(matches, engine)
      // 0.13.71 evidence_ledger_v1 — repeat-glob nudge on successful runs.
      if (!result.isError && context?.taskState) {
        const matchCount = matches.length
        const nudge = recordGlobAndBuildNudge(context.taskState, input, matchCount)
        if (nudge.prefix) {
          result.output = `${nudge.prefix}\n${result.output}`
          result.metadata = {
            ...(result.metadata ?? {}),
            globRepeatCount: nudge.count,
          }
        }
      }
      return result
...
function successResult(matches: string[], engine: 'ripgrep' | 'native'): ToolResult {
  ...
  if (sorted.length === 0) {
    return {
      ...
      isError: false,
      metadata: { count: 0, engine, truncated: false },
    }
  }
  const display = displayMatches(sorted)
  const totalCount = sorted.length
  const cappedAtCollection = totalCount >= HARD_RESULT_CAP
  const truncated = display.truncated || cappedAtCollection
  const metadata: Record<string, unknown> = {
    count: display.returnedCount,
    engine,
    truncated,
  }
  if (truncated) {
    metadata.returnedCount = display.returnedCount
    metadata.totalCount = totalCount
    metadata.collectionCapped = cappedAtCollection
  }
  return {
    output: display.text,
    isError: false,
...
function partialResult(
  ...
  const display = sorted.length > 0
    ...
    : { text: 'No files matched before the run stopped.', truncated: false, returnedCount: 0 }
  const totalCount = sorted.length
  const cappedAtCollection = totalCount >= HARD_RESULT_CAP
  const truncated = display.truncated || cappedAtCollection
  const metadata: Record<string, unknown> = {
...
function displayMatches(matches: string[]): DisplayMatches {
  ...
  const sentinel = `... (truncated: ${matches.length} total, showing first ${MAX_RESULTS})`
  return {
    text: [...shown, sentinel].join('\n'),
    truncated: true,
    returnedCount: MAX_RESULTS,
  }
}
