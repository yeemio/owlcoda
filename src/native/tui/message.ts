...
import { truncate, padRight, hardWrap } from './text.js'
...
const FOOTER_ONLY_SUCCESS_TOOLS = new Set(['DeliveryAudit', 'TaskCreate', 'TaskUpdate', 'TaskOutput'])
...
const TOOL_NAME_ALIASES: Record<string, string> = {
  ...
  artifactverify: 'ArtifactVerify',
  deliveryaudit: 'DeliveryAudit',
  structuredoutput: 'StructuredOutput',
  skillroutepreview: 'SkillRoutePreview',
  brief: 'Brief',
}
...
function canonicalToolName(name: string): string {
  ...
  return TOOL_NAME_ALIASES[lower] ?? name
}

export function userFacingToolName(name: string): string {
  const canonical = canonicalToolName(name)
  return USER_FACING_TOOL_NAMES[canonical] ?? canonical
}
...
export function shouldRouteToolStartFooterOnly(name: string): boolean {
  return FOOTER_ONLY_SUCCESS_TOOLS.has(name)
}

export function shouldRouteToolEndFooterOnly(name: string, isError: boolean): boolean {
  return !isError && FOOTER_ONLY_SUCCESS_TOOLS.has(name)
}

export function formatFooterOnlyToolStart(name: string): string {
  return dim(`${userFacingToolName(name)} running…`)
}

export function formatFooterOnlyToolEnd(name: string, durationMs: number): string {
  return dim(`${userFacingToolName(name)} completed (${formatDuration(durationMs)})`)
}
...
export interface ComposerRailOptions {
  ...
  /**
   * True when `autoApprove` is on (env var or `/yolo`/`/approve on`).
   * In yolo mode every tool call — including `dangerous` bash like
   * `rm -rf /` — runs without a permission prompt. The rail surfaces
   * a red `YOLO` cell so the user can't forget the safety net is off.
   */
...
export function renderComposerRail(opts: ComposerRailOptions): string {
  ...
  if (opts.yolo) {
    // Slot directly after `state` so the YOLO marker stays visible even
    // under tight width pruning — the tail-drop loop below pops cells from
    // the right, and dropping the YOLO indicator while dangerous-bypass is
    // active is exactly what we don't want.
    cells.splice(1, 0, {
      kind: 'kv',
      label: 'mode',
      value: 'YOLO',
...
function formatContextPressure(tokens: number, max: number): string {
  // Shared canonical formatter so the status-bar CTX cell renders large
  // windows as "2M" rather than the misread-prone "2097.2k", and stays
  // consistent with the model-window labels.
  return `${formatTokenCompact(tokens)}/${formatTokenCompact(max)}`
}
...
function summarizeToolInput(name: string, input: Record<string, unknown>): string {
  switch (canonicalToolName(name)) {
    ...
    case 'TodoWrite':
      return summarizeTodoWriteInput(input)
    case 'Brief':
      return summarizeBriefInput(input)
    case 'TaskCreate':
      return summarizeTaskCreateInput(input)
    case 'TaskUpdate':
      return summarizeTaskUpdateInput(input)
    case 'TaskVerify':
      return summarizeTaskVerifyInput(input)
    case 'TaskGet':
    case 'TaskOutput':
    case 'TaskStop':
      return summarizeTaskIdInput(input)
    case 'TaskList':
      return 'list tasks'
    case 'AskUserQuestion':
      return summarizeStringField(input, 'question', 'question')
    case 'RunWorkspace':
      return summarizeRunWorkspaceInput(input)
    case 'ArtifactVerify':
      return summarizeArtifactVerifyInput(input)
    case 'DeliveryAudit':
      return summarizeDeliveryAuditInput(input)
    case 'StructuredOutput':
      return summarizeStructuredOutputInput(input)
    case 'SkillRoutePreview':
      return summarizeStringField(input, 'prompt', 'route preview')
    default:
      return summarizeGenericToolInput(input)
  }
}

function summarizeTodoWriteInput(input: Record<string, unknown>): string {
...
function summarizeBriefInput(input: Record<string, unknown>): string {
  const message = compactValue(input['message'], 72) || 'message'
  const attachments = Array.isArray(input['attachments']) ? input['attachments'].length : 0
  if (attachments <= 0) return message
  return `${message} · ${attachments} attachment${attachments === 1 ? '' : 's'}`
}

function summarizeTaskCreateInput(input: Record<string, unknown>): string {
  const subject = compactValue(input['subject'], 72) || compactValue(input['description'], 72) || 'create task'
  const steps = Array.isArray(input['steps']) ? input['steps'].length : 0
  const command = compactValue(input['command'], 60)
  const parts = [subject]
...
function summarizeTaskUpdateInput(input: Record<string, unknown>): string {
  const taskId = compactValue(input['taskId'], 32)
  const stepId = compactValue(input['stepId'], 32)
  const status = compactValue(input['stepStatus'] ?? input['status'], 32)
  const parts: string[] = []
  if (taskId) parts.push(taskId)
  if (stepId) parts.push(stepId)
  if (status) parts.push(`to ${status}`)
  return parts.length > 0 ? parts.join(' · ') : 'update task'
}

function summarizeTaskVerifyInput(input: Record<string, unknown>): string {
  const taskId = compactValue(input['taskId'], 32)
  const stepId = compactValue(input['stepId'], 32)
  const parts = ['verify']
  if (taskId) parts.push(taskId)
...
function summarizeTaskIdInput(input: Record<string, unknown>): string {
  return compactValue(input['taskId'], 64) || 'task'
}

function summarizeRunWorkspaceInput(input: Record<string, unknown>): string {
  const action = compactValue(input['action'] ?? input['mode'], 32)
  const outputRoot = compactValue(input['outputRoot'] ?? input['root'] ?? input['path'], 80)
  if (action && outputRoot) return `${action} · ${outputRoot}`
  return action || outputRoot || 'run workspace'
}

function summarizeArtifactVerifyInput(input: Record<string, unknown>): string {
  const path = compactValue(input['path'] ?? input['artifactPath'] ?? input['file'], 80)
  const kind = compactValue(input['kind'] ?? input['artifactType'], 32)
  if (path && kind) return `${kind} · ${path}`
  return path || kind || 'verify artifact'
}

function summarizeDeliveryAuditInput(input: Record<string, unknown>): string {
  const claims = Array.isArray(input['claims']) ? input['claims'].length : 0
  const files = Array.isArray(input['expectedFiles']) ? input['expectedFiles'].length : 0
  const parts: string[] = []
  if (claims > 0) parts.push(`${claims} claim${claims === 1 ? '' : 's'}`)
  if (files > 0) parts.push(`${files} file${files === 1 ? '' : 's'}`)
  return parts.length > 0 ? parts.join(' · ') : 'audit delivery'
}

function summarizeStructuredOutputInput(input: Record<string, unknown>): string {
  const schema = input['schema'] !== undefined ? 'schema' : ''
  const data = input['data'] !== undefined ? 'data' : ''
  return [schema, data].filter(Boolean).join(' + ') || summarizeGenericToolInput(input)
}

function summarizeStringField(input: Record<string, unknown>, field: string, fallback: string): string {
  return compactValue(input[field], 80) || fallback
}

function summarizeGenericToolInput(input: Record<string, unknown>): string {
  const entries = Object.entries(input).filter(([, value]) => value !== undefined)
  if (entries.length === 0) return ''
  return entries
    .slice(0, 3)
    .map(([key, value]) => {
      const compact = compactValue(value, 48)
      return compact ? `${key}: ${compact}` : key
    })
    .join(' · ')
}

function compactValue(value: unknown, maxChars: number): string {
...
function wrapDisplayLine(text: string, maxWidth: number): string[] {
  const wrapped = hardWrap(text, maxWidth)
