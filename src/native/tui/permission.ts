...
/**
 * Render a TaskCreate permission dialog with command preview.
 *
 * TaskCreate may carry a `command` to spawn. When it does, the operator
 * should see the same risk verdict they would for `bash` — both surfaces
 * route through `classifyBashCommand`, so the warning copy and border
 * color are identical to `renderBashPermission`. Without a command this
 * is a pure TODO entry and never asks for approval (the call is allowed
 * by the headless policy without prompting).
 */
export function renderTaskCreatePermission(
  command: string,
  cwd?: string,
  selectedIndex = 1,
): string {
  ...
  const borderColor = warning ? 'error' : 'bashBorder'
  return renderTopBorderDialog({
    title: `🔐 ${themeColor(borderColor)}TaskCreate${sgr.reset}`,
    color: borderColor,
    content,
  })
}
