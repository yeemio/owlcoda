...
import { buildFidelityTelemetrySummary, formatFidelityReviewPacket, formatFidelityTelemetryJson, formatFidelityTelemetrySummary } from "./fidelity-telemetry-summary.js"
...
export type SlashCommandSideEffect =
  | { kind: 'force_resubmit_failed_turn'; text: string }
  | { kind: 'open_editor'; initialContent: string }
  ...
  if (shouldOpenBrowser) {
    ...
    if (bundle.available) {
      ...
      if (opened) {
        ...
        // macOS `open` only tells us the handoff command was accepted. Keep a
        // visible URL fallback for picker-launched Admin, where a missing
        // browser window otherwise looks like a dead Enter key.
      } else {
...
export interface ApproveState {
  autoApprove: boolean
  /**
   * When true, every prompt that would normally fire is auto-resolved
   * to "deny" instead of awaiting user input. Mutually exclusive with
   * `autoApprove` (autoApprove wins on conflict). Mostly a QA aid:
   * autonomous flows can use this to verify the safety gate is firing
   * (the tool call will return "Tool execution denied by user." instead
   * of running silently). Set via `Config({setting:'autoDeny', value:true})`.
   */
  autoDeny?: boolean
}
...
function projectMapStatusCwd(conversation: Conversation): string {
  return conversation.options?.taskState?.contract.cwd ?? process.cwd()
}

function renderProjectMapStatus(
  ...
  const entrypoints = snapshot.entrypoints
    .slice(0, 6)
    .map((entry) => `${displayProjectMapPath(entry.path, root)} (${entry.kind})`)
  const boundaries = snapshot.writeBoundaries
    .slice(0, 6)
    .map((boundary) => `${boundary.kind} ${displayProjectMapPath(boundary.path, root)} (${boundary.origin})`)
  const verification = snapshot.verificationProfiles
    .flatMap((profile) => profile.commands)
    .slice(0, 6)

  return [
    `${ansi.bold}Project map:${ansi.reset}${options.refreshed ? ' refreshed' : ''}`,
    `  Enabled:   ${isProjectMapEnabled() ? 'yes' : 'no'} (default-on; OWLCODA_PROJECT_MAP=0 disables)`,
    `  Snapshot:  ${presence}`,
    `  Package:   ${packageLabel}`,
    `  Sources:   ${formatProjectMapSourceCounts(snapshot)}`,
    `  Entry:     ${formatProjectMapList(entrypoints)}`,
    `  Boundary:  ${formatProjectMapList(boundaries)}`,
    `  Verify:    ${formatProjectMapList(verification, '; ')}`,
  ].join('\n')
}

function formatProjectMapSourceCounts(snapshot: ProjectMapSnapshot): string {
...
function displayProjectMapPath(filePath: string, root: string): string {
...
function formatProjectMapList(values: string[], separator = ', '): string {
...
export function parseApiError(raw: string): string {
  ...
  // Curated runtime/guard messages are multi-line and pre-formatted (e.g. the
  // no-progress hard-stop reason carries actionable "What to do" guidance well
  // past 200 chars). Only the single-line raw-blob case needs the tight cap to
  // avoid dumping a huge minified JSON error; multi-line messages pass through,
  // bounded generously so a pathological blob is still capped.
  if (raw.includes('\n')) {
...
${isModesEnabled() ? `    /mode [mode]      Approval mode (${OPERATING_MODES.join('|')}); plan/yolo are aliases\n` : ''}    /vim              Toggle vim keybindings
  ...
  ${sgr.bold}Observability:${sgr.reset}
    ...
    /fidelity [json|review] Show evidence-ledger fidelity telemetry
...
`)
    ...
    case '/retry': {
      ...
      if (hooks.onSideEffect) {
        hooks.onSideEffect({ kind: 'force_resubmit_failed_turn', text: lastUserText })
      } else if (rl) {
    ...
    case '/editor': {
      if (!hooks.onSideEffect) {
        console.log(dim('Editor is only available in the interactive REPL.'))
        return true
      }
      hooks.onSideEffect({ kind: 'open_editor', initialContent: arg })
      return true
    }
    ...
    case '/budget': {
      ...
      const snapshot = buildContextBudgetSnapshot(conversation, contextWindow, {
        outputMaxTokens: conversation.maxTokens,
      })
      console.log(formatBudget(snapshot.usedTokens, snapshot.contextWindow, {
        outputMaxTokens: snapshot.outputMaxTokens,
        breakdown: snapshot.breakdown,
        thresholdRatio: snapshot.thresholdRatio,
    ...
    case '/project-map': {
      if (arg && arg !== 'refresh') {
        console.log(`${ansi.red}Usage: /project-map [refresh]${ansi.reset}`)
        return true
      }

      const cwd = projectMapStatusCwd(conversation)
      if (arg === 'refresh') {
        const snapshot = buildProjectMapSnapshot(cwd)
        if (!conversation.options) conversation.options = {}
        conversation.options.projectMapSnapshot = snapshot
        conversation.options.projectMapPromptSummary = buildProjectMapPromptSummary(snapshot)
        console.log(renderProjectMapStatus(snapshot, 'present', { refreshed: true }))
        return true
      }

      const snapshot = conversation.options?.projectMapSnapshot ?? buildProjectMapSnapshot(cwd)
      const presence = conversation.options?.projectMapSnapshot ? 'present' : 'absent (live read-only scan)'
      console.log(renderProjectMapStatus(snapshot, presence, { refreshed: false }))
      return true
    }
    ...
    case '/config': {
      ...
      if (isModesEnabled()) {
        lines.push(`  Operating mode:  ${operatingMode}`)
      }
      if (opts) {
      ...
      lines.push(
        ...
        `  Config file:     ${getOwlcodaConfigLabel()}`,
        '',
    ...
    case '/mode': {
      if (!isModesEnabled()) {
        console.log(dim('Permission modes are disabled by OWLCODA_MODES=0. Unset it or set OWLCODA_MODES=1 to enable /mode.'))
        return true
      }
    ...
    case '/fidelity': {
      const summary = buildFidelityTelemetrySummary()
      if (arg === 'review' || arg === '--review') {
        console.log(formatFidelityReviewPacket(summary))
      } else {
        const json = arg === 'json' || arg === '--json'
        console.log(json ? formatFidelityTelemetryJson(summary) : formatFidelityTelemetrySummary(summary))
      }
      return true
    }
    ...
    case '/context': {
      ...
      const { totalTokens, systemTokens } = breakdown
      const contextCapability = resolveModelContextCapability(loadConfig(), conversation.model)
      const maxTokens = contextCapability.contextWindow > 0 ? contextCapability.contextWindow : 1
      ...
      console.log(`  Capability: ${contextCapability.labels.join(' · ')}`)
      if (contextCapability.notes.length > 0) {
        console.log(`  ${dim(contextCapability.notes.join(' '))}`)
      }
      console.log('')
      ...
      console.log(`  ${dim(`User messages:`)}  ~${(breakdown.userTextTokens / 1000).toFixed(1)}K tokens`)
      console.log(`  ${dim(`Assistant:`)}      ~${(breakdown.assistantTextTokens / 1000).toFixed(1)}K tokens`)
      console.log(`  ${dim(`Tool calls:`)}     ~${(breakdown.toolUseTokens / 1000).toFixed(1)}K tokens`)
      console.log(`  ${dim(`Tool evidence:`)}  ~${(breakdown.toolResultTokens / 1000).toFixed(1)}K tokens`)
      console.log(`  ${dim(`Output cap:`)}      ${conversation.maxTokens.toLocaleString()} max_tokens (generation only)`)
    ...
    case '/btw': {
      ...
      try {
        ...
        const { finalText, usage: sideUsage, stopReason, runtimeFailure } = await runConversationLoop(sideConvo, dispatcher!, {
          ...
          contextWindow: resolveModelContextWindow(loadConfig(), sideConvo.model),
          callbacks: {
            onText(text) {
      ...
      for (const [label, dir] of [['Project', projectSkillDir], ['User', userSkillDir]] as const) {
        try {
          ...
          for (const entry of entries.sort()) {
            ...
            const isMarkdownFile = entry.endsWith('.md') || entry.endsWith('.txt')
            const isSkillPackage = statSync(fullPath).isDirectory() && existsSync(join(fullPath, 'SKILL.md'))
            if (!isMarkdownFile && !isSkillPackage) continue

            foundFileSkills = true
            const skillName = isSkillPackage
              ? entry
              : basename(entry, entry.endsWith('.md') ? '.md' : '.txt')
    ...
    case '/tasks': {
      ...
      const statusIcon = (s: string): string =>
        s === 'completed' ? '✓' : s === 'in_progress' ? '▶' : s === 'cancelled' ? '✗' : s === 'blocked' ? '⏸' : '○'
      const lines: string[] = [`${ansi.bold}Tasks:${ansi.reset}`, '']
      if (agents.size > 0) {
        lines.push(`  ${ansi.bold}Running agents${ansi.reset}`)
        const now = Date.now()
        for (const [id, info] of agents) {
          const elapsed = Math.round((now - info.startTime) / 1000)
          lines.push(`    ⚡ ${id} — ${info.description} (${elapsed}s)`)
        }
        lines.push('')
      }
      if (tasks.length > 0) {
        lines.push(`  ${ansi.bold}Tasks${ansi.reset}`)
        for (const t of tasks) {
          const exit = t.command && t.exitCode !== undefined ? ` (exit ${t.exitCode})` : ''
    ...
    case '/hooks': {
      const plugins = getLoadedPlugins()
      if (plugins.length === 0) {
        console.log(`${ansi.dim}No plugins loaded → no active hooks. Place plugins in ~/.owlcoda/plugins/<name>/index.js${ansi.reset}`)
        return true
      }
      const RUNTIME = ['onRequest', 'onResponse', 'onToolCall', 'onError']
      const LIFECYCLE = ['onLoad', 'onUnload']
      const lines: string[] = [`${ansi.bold}Plugin Hooks:${ansi.reset}`, '']
      const renderGroup = (title: string, methods: string[]) => {
        lines.push(`  ${ansi.bold}${title}${ansi.reset}`)
        for (const m of methods) {
          const impls = plugins
            .filter((p) => typeof (p.plugin as unknown as Record<string, unknown>)[m] === 'function')
            .map((p) => p.plugin.metadata.name)
          lines.push(`    ${m.padEnd(12)} ${impls.length ? impls.join(', ') : `${ansi.dim}—${ansi.reset}`}`)
        }
        lines.push('')
      }
      renderGroup('Runtime hooks', RUNTIME)
      renderGroup('Lifecycle hooks', LIFECYCLE)
      for (const p of plugins) {
        const n = [...RUNTIME, ...LIFECYCLE].filter((m) => typeof (p.plugin as unknown as Record<string, unknown>)[m] === 'function').length
        lines.push(`  📦 ${p.plugin.metadata.name} v${p.plugin.metadata.version} (${n} hooks)`)
      }
      console.log(lines.join('\n'))
      return true
    }
    ...
    case '/training': {
      // Local training-data surface. Reads ONLY — never appends, never clears.
      // /training export prints a CLI hint instead of dumping the full dataset
      // (which can be hundreds of MB) into the REPL.
      const sub = (parts[1] ?? 'status').toLowerCase()
      const store = await import('../data/training-store.js')

      if (sub === 'path') {
        console.log(store.getCollectedPath())
        return true
      }

      if (sub === 'export') {
        const cmd = `owlcoda training export jsonl --sanitize --limit 100 > /tmp/owlcoda-training-sample.jsonl`
        console.log(`\n  ${themeColor('info')}Use the CLI to export training data — full dataset can be very large.${sgr.reset}`)
        console.log(`  ${dim('Suggested:')} ${cmd}`)
        console.log(`  ${dim('See also:')} owlcoda training export jsonl --sanitize  (full dataset)\n`)
        return true
      }

      if (sub === 'sample') {
        const status = await store.readCollectedStatus()
        if (status.lineCount === 0) {
          console.log(`\n  ${dim('No collected training data yet.')}\n`)
          console.log(`  ${dim('File:')} ${status.path}\n`)
          return true
        }
        const samples = await store.readCollectedSample({ limit: 2, maxChars: 240, sanitize: true })
        console.log(`\n  ${themeColor('owl')}🦉 Training Sample${sgr.reset} ${dim('(sanitized, truncated)')}\n`)
        for (const s of samples) {
          console.log(`  ${s.raw}${s.truncated ? dim(' …') : ''}`)
        }
        console.log()
        return true
      }
      ...
      if (sub !== 'status' && sub !== '' && sub !== undefined) {
        console.log(`\n  ${dim('Unknown subcommand:')} /training ${sub}`)
        console.log(`  ${dim('Available:')} /training [status|path|sample|export]\n`)
        return true
      }
      ...
      const r = sgr.reset
      console.log(`\n  ${owl}📦 Training Data${r}\n`)
      if (!status.manifestPresent && status.lineCount === 0) {
        console.log(`  ${dim('No training data collected yet.')}`)
        console.log(`  ${dim('File:')} ${status.path}`)
        console.log(`  ${dim('Tip: enable trainingCollection in config to auto-collect high-quality sessions.')}\n`)
      } else {
        ...
        console.log(`\n  ${dim('Subcommands:')} /training [status|path|sample|export]\n`)
      }
      return true
    }
