 ...
 */
// Redirect targets that consume output without mutating the workspace:
// the bit-bucket and the std streams. Mirrors the /dev/null exclusion in
// task-state.ts's extractBashArtifactPaths so the two write-detectors agree.
const NON_MUTATING_REDIRECT_TARGETS = new Set(['/dev/null', '/dev/stdout', '/dev/stderr'])

function isNonMutatingRedirectTarget(target: string): boolean {
  const cleaned = target.replace(/^['"]+|['"]+$/g, '')
  return NON_MUTATING_REDIRECT_TARGETS.has(cleaned) || /^\/dev\/fd\/\d+$/.test(cleaned)
}
...
function detectMutatingRedirect(chunk: string): string | null {
  ...
  for (let i = 0; i < chunk.length; i++) {
    ...
    if (ch === '<') continue
    if (ch === '>') {
      // Skip `2>&1` style fd duplication: if next is `&`, this is a
      // duplication, not a write to a file.
      if (next === '&') continue
      // Read the redirect target. Writing to /dev/null (and the other
      // bit-bucket / std-stream sinks) does NOT mutate the workspace —
      // e.g. `find ... 2>/dev/null` is a pure read. Dangerous device
      // redirects (`> /dev/sd*`, `> /etc/`) are caught earlier by
      // DANGEROUS_PATTERNS, so they never reach this exclusion.
      let j = i + 1
      if (chunk[j] === '>') j++ // `>>` append
      while (j < chunk.length && (chunk[j] === ' ' || chunk[j] === '\t')) j++
      let target = ''
      while (j < chunk.length && !' \t\n;&|)'.includes(chunk[j]!)) { target += chunk[j]!; j++ }
      if (isNonMutatingRedirectTarget(target)) { i = j - 1; continue }
...
function classifySafeRead(chunk: string): ChunkVerdict | null {
  ...
  // don't have any unexpected suffix). `sleep` is side-effect-free and
  // shows up in legitimate task patterns like `sleep 5; echo done`,
