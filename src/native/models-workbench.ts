...
export function renderOverview(snapshot: ModelTruthSnapshot, tier: WidthTier): string[] {
  ...
  lines.push(`  ${dim(`config source: ${getOwlcodaConfigLabel()} (local user profile, not bundled)`)}`)
  lines.push('')
...
export async function runModelsWorkbench(opts: RunWorkbenchOptions = {}): Promise<void> {
  ...
  if (sorted.length === 0) {
    stream.write(dim(`  no models configured — add models to ${getOwlcodaConfigLabel()}\n`))
    return
  }
...
function buildTitleSummary(def: ModelStatus | undefined, counts: OverviewCounts, tier: WidthTier): string {
  ...
  const config = tier === 'narrow' ? '' : ` · cfg ${getOwlcodaConfigLabel()}`
  return `🦉 Models · ${defText} · ${tail}${config}`
}
