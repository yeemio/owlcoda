...
export const NATIVE_TOOL_SCHEMAS: Record<string, Record<string, unknown>> = {
      ...
      todos: {
        ...
        items: {
          ...
          properties: {
            ...
            failureReason: { type: 'string', description: 'Reason this todo is blocked or skipped; required for blocked/skipped statuses.' },
          },
          ...
          properties: {
            ...
            verification: {
              ...
              items: {
                ...
                properties: {
                  id: { type: 'string' },
                  kind: { type: 'string', enum: ['file_exists', 'file_contains', 'artifact_count', 'command', 'http_get', 'verification_pack', 'run_verdict_gate', 'mcnemar_method', 'none'] },
                  path: { type: 'string' },
                  pattern: { type: 'string' },
                  root: { type: 'string' },
                  glob: { type: 'string' },
                  min: { type: 'number' },
                  discordant01: { type: 'number', description: 'McNemar 0→1 discordant count.' },
                  discordant10: { type: 'number', description: 'McNemar 1→0 discordant count.' },
                  requestedMethod: { type: 'string', enum: ['exact', 'asymptotic'] },
                  command: { type: 'string' },
                  expectedExitCode: { type: 'number' },
                  url: { type: 'string', description: 'URL for http_get verification checks. Only localhost/127.0.0.1/::1 are allowed.' },
                  expectedStatus: { type: 'number', description: 'Expected HTTP status for http_get checks (default 200).' },
                  bodyPattern: { type: 'string', description: 'Optional regex that must match the http_get response body.' },
    ...
    properties: {
      ...
      verification: {
        ...
        items: {
          ...
          properties: {
            id: { type: 'string' },
            kind: { type: 'string', enum: ['file_exists', 'file_contains', 'artifact_count', 'verification_pack', 'run_verdict_gate', 'mcnemar_method', 'http_get', 'command', 'none'] },
            packId: { type: 'string' },
            path: { type: 'string' },
            pattern: { type: 'string' },
            root: { type: 'string' },
            glob: { type: 'string' },
            min: { type: 'number' },
            discordant01: { type: 'number' },
            discordant10: { type: 'number' },
            requestedMethod: { type: 'string', enum: ['exact', 'asymptotic'] },
  ...
  ReadMcpResource: {
    ...
    properties: {
      server: { type: 'string', description: 'MCP server name' },
      uri: { type: 'string', description: 'Resource URI to read. Local file:// URIs and absolute local paths are automatically routed through the native read tool.' },
    },
    required: ['server', 'uri'],
  },
  McpAuth: {
