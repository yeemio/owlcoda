...
export class ModelTruthAggregator {
  ...
  private async buildSnapshot(): Promise<Omit<ModelTruthSnapshot, keyof ModelTruthState>> {
    ...
    for (const status of entries.values()) {
      ...
      const contextCapability = resolveSeedContextCapability(status)
...
function resolveSeedContextCapability(model: SeedModel): ModelContextCapability {
  const configuredWindow = model.config?.contextWindow
  const discoveredWindow = model.discovered?.contextWindow
  const useRuntimeDiscovered = typeof discoveredWindow === 'number'
    && (configuredWindow === undefined || configuredWindow === DEFAULT_CONTEXT_WINDOW)

  return resolveModelCapabilities({
    id: model.id,
    label: model.label,
    backendModel: presentBackendModel(model),
    aliases: model.aliases,
    endpoint: model.endpoint,
    contextWindow: useRuntimeDiscovered ? discoveredWindow : configuredWindow,
    contextWindowSource: useRuntimeDiscovered ? 'runtime_discovered' : undefined,
  }).context
}

function presentBackendModel(model: SeedModel): string {
  return model.config?.backendModel ?? model.discovered?.id ?? model.backendModel
}
