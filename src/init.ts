...
import { normalizeRouterBaseUrl } from './config.js'
...
export type RouterModelFetcher = (url: string) => Promise<unknown>

export async function detectRouterModels(
  routerUrl: string,
...
function buildConfigFromModels(models: string[], routerUrl: string, port: number): Record<string, unknown> {
  ...
  // No detected models → write an empty models[]. The historical
  // your-default-model placeholder caused two real Windows failures: a sticky
  // local default with no endpoint blocked launch on local-runtime preflight,
  // and persisted across upgrades so users who later added a cloud provider
  // still routed through the local path. Empty models[] lets doLaunch route
  // straight to Admin/start onboarding.
  return {
    port,
    host: '127.0.0.1',
    routerUrl,
...
export async function runInit(options: InitOptions = {}): Promise<InitResult> {
  ...
  // Resolve endpoint + models. If the user passed --endpoint (or the legacy
  // --router alias), honor it as their explicit choice (single probe).
  // Otherwise sweep the well-known local OpenAI-compatible endpoints and pick
  // the first responder.
  ...
  if (options.routerUrl) {
    routerUrl = normalizeRouterBaseUrl(options.routerUrl)
  ...
  const modelMsg = detectedModels.length > 0
    ...
    : `No local backend reachable. Add a cloud provider in Admin (run 'owlcoda admin') or start a local runtime, then re-launch.`
...
export function formatInitResult(result: InitResult): string {
  ...
  if (result.created) {
    ...
    if (!result.modelsDetected) {
      lines.push('  1. Run: owlcoda admin    — add a cloud provider (MiniMax, OpenRouter, …) or wire a local runtime')
    }
