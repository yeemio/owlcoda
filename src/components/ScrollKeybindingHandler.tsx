// @ts-nocheck — generated/ported terminal scroll handler; types are intentionally loose
...
// event/notch — users on those can set OWLCODA_SCROLL_SPEED=3 to match
...
/** Read OWLCODA_SCROLL_SPEED, default 1, clamp (0, 20].
 ...
 *  set OWLCODA_SCROLL_SPEED=3 to match vim/nvim/opencode. We can't
...
export function readScrollSpeedBase(): number {
  const raw = process.env.OWLCODA_SCROLL_SPEED;
  if (!raw) return 1;
...
export function ScrollKeybindingHandler({
  ...
  //
  // TODO(search): `/`, n/N. getItemY Yoga-walk + computeOrigin + anchorY
  // already solve scroll-to-index. jumpToPrevTurn is the n/N template.
  // Single-shot via OVERSCAN_ROWS=80; two-phase was tried and abandoned
  // (oscillation). See team memory scroll-copy-mode-design.md.
