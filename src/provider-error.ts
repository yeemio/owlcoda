...
export type ProviderFailureKind =
   ...
   *  remainder. */
  | 'stream_interrupted'
  /** 0.13.97: first-token watchdog fired — the stream stayed open but no
   *  visible chunk arrived within `middleware.streamFirstTokenTimeoutMs`.
   *  Distinct from `stream_interrupted_before_first_token` (which means the
   *  upstream connection actually closed). User has nothing yet, so
   *  recovery is "switch model / retry"; partialOutputSeen is always false. */
  | 'stream_first_token_timeout'
  /** 0.13.97: between-chunks idle watchdog fired AFTER first-token had
   *  arrived. Upstream may still be alive but went silent past
   *  `middleware.streamIdleTimeoutMs` (or per-route `timeoutMs`).
   *  partialOutputSeen is true by definition; transcript already shows
   *  whatever arrived. */
  | 'stream_idle_timeout'
  | 'unknown_fetch_error'
...
export interface ProviderRequestDiagnostic {
  ...
  /** 0.13.97: true when the proxy saw at least one content_block_delta /
   *  content_block_start before the failure. Used by frontend recovery
   *  guidance to distinguish "nothing reached the user" (suggest /model)
   *  from "partial output is in transcript" (suggest follow-up). Defaults
   *  to undefined for diagnostics built outside the streaming loop. */
...
/**
 * Some providers wrap rate-limit / quota failures in HTTP 400 instead of the
 * standard 429 (observed in production with mimo-v25-pro's
 * `Cluster rate limit exceeded, request queued but not admitted` shape).
 * When the detail text clearly indicates a rate-limit-class failure, treat
 * the response as retryable so the conversation auto-retry path can recover
 * without exposing a hard error to the user.
 *
 * Matches both English and Chinese phrasings, and several common
 * cluster/queue/quota signals across providers.
 */
const RATE_LIMIT_DETAIL_PATTERNS: RegExp[] = [
  /\brate[\s_-]?limit(?:ed|s)?\b/i,
  /\btoo many requests\b/i,
  /\bquota\b.*\b(exceeded|insufficient|limit)\b/i,
  /\b(cluster|node|tenant)\b.*\b(limit|saturat|capacity)\b/i,
  /\bqueued?\b.*\bnot admitted\b/i,
  /\bnot admitted\b/i,
  /\bthrottl(ed|ing)\b/i,
  /(限流|频率限制|请求过多|额度不足|超出配额|超过配额|超出限制|被限速|排队但未被接纳)/,
]

export function detailLooksLikeRateLimit(rawDetail: string | undefined | null): boolean {
  if (!rawDetail) return false
  return RATE_LIMIT_DETAIL_PATTERNS.some((re) => re.test(rawDetail))
}
...
export function createProviderHttpDiagnostic(
  ...
  // 2026-05-28: detail-text inference for misclassified rate-limits.
  // mimo-v25-pro wraps cluster rate-limit rejections in HTTP 400 instead of
  // 429, which previously left `retryable: false` and surfaced as a hard
  // failure for fan-out sub-agent calls. Detail-text detection lets us treat
  // these as retryable without changing the kind union or downstream
  // consumers — the existing native auto-retry path picks it up.
...
/** 0.13.97: first-token watchdog fired — upstream stayed open but produced
 *  no visible chunk within `streamFirstTokenTimeoutMs`. Always
 *  `partialOutputSeen: false` by definition. */
export function createStreamFirstTokenTimeoutDiagnostic(
  context: ProviderDiagnosticContext = {},
  options: { timeoutMs: number },
): ProviderRequestDiagnostic {
  const provider = inferProviderName(context)
  const model = context.model ?? 'unknown-model'
  const detail = `first-token watchdog fired after ${options.timeoutMs}ms with no visible chunk`
  return {
    provider,
    model,
...
/** 0.13.97: between-chunks idle watchdog fired AFTER first-token already
 *  arrived. Always `partialOutputSeen: true` by definition. */
export function createStreamIdleTimeoutDiagnostic(
  context: ProviderDiagnosticContext = {},
  options: { timeoutMs: number },
): ProviderRequestDiagnostic {
  const provider = inferProviderName(context)
  const model = context.model ?? 'unknown-model'
  const detail = `idle watchdog fired after ${options.timeoutMs}ms with partial output already streamed`
  return {
    provider,
    model,
...
function defaultStatusForKind(kind: ProviderFailureKind): number {
  switch (kind) {
    case 'timeout':
    case 'stream_first_token_timeout':
