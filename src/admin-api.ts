...
import { getProviderTemplates, ProviderProbe, type DryRunProviderPayload } from './provider-probe.js'
import { discoverLocalRuntimes, type DiscoveredRuntime } from './localhost-discovery.js'
import { EndpointHealthCache, sampleFromProbeResult } from './endpoint-health-cache.js'
import {
  ModelConfigMutator,
...
export interface AdminApiDeps {
  ...
  discoverLocalRuntimes?: () => Promise<DiscoveredRuntime[]>
  ...
  try {
    switch (route.action) {
      ...
      case 'providers':
        ...
        return true
      case 'local-runtimes': {
        const runtimes = await (deps.discoverLocalRuntimes ?? discoverLocalRuntimes)()
        sendAdminJson(res, 200, {
          schemaVersion: ADMIN_API_SCHEMA_VERSION,
          runtimes,
        })
...
function matchRoute(method: string, url: string): RouteMatch | null {
  ...
  if (method === 'GET' && url === '/admin/api/local-runtimes') return { action: 'local-runtimes' }
  if (method === 'GET' && url === '/admin/api/endpoint-health') return { action: 'endpoint-health' }
...
export function sanitizeSnapshot(
  ...
  return {
    ...snapshot,
    configSourceLabel: getOwlcodaConfigLabel(),
