...
export function generateZshCompletion(): string {
  const cmdDescs = [
    ...
    'health:Check proxy and endpoint health',
...
${cmdDescs.map(d => `    '${d}'`).join('\n')}
  ...
  flags=(
    ...
    '--endpoint[Override default endpoint URL]:url:'
    '--router[Legacy alias for --endpoint]:url:'
    '--model[Select model]:model:'
    '--mode[Initial operating mode unless OWLCODA_MODES=0]:(plan normal auto)'
...
export function generateFishCompletion(): string {
  ...
  const flagDescs: Array<[string, string, boolean]> = [
    ...
    ['endpoint', 'Override default endpoint URL', true],
    ['router', 'Legacy alias for --endpoint', true],
    ['model', 'Select model', true],
    ['mode', 'Initial operating mode unless OWLCODA_MODES=0', true],
