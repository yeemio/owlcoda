...
import { loadConfig, type OwlCodaConfig, listConfiguredModels, getDefaultConfiguredModel } from './config.js'
...
export function getConfigDisplay(configPath?: string): ConfigDisplay {
  ...
  const effectiveDefaultId = config ? getDefaultConfiguredModel(config)?.id : undefined
  const models = config ? listConfiguredModels(config).map(m => {
    const capabilities = resolveModelCapabilities(m)
    return {
      id: m.id,
      label: m.label || m.id,
...
export function formatConfigDisplay(display: ConfigDisplay): string {
  ...
  if (display.models.length > 0) {
    ...
    for (const m of display.models) {
      ...
      if (!m.isDefault && m.configuredDefault) tags.push('configured default ignored')
      if (m.tier) tags.push(m.tier)
      if (m.aliases.length > 0) tags.push(`aliases: ${m.aliases.join(', ')}`)
      tags.push(...(m.contextLabels ?? []))
      tags.push(...(m.capabilityLabels ?? []))
