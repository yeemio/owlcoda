...
export class MeasuredText {
  ...
  private measureWrappedText(): WrappedLine[] {
    // 0.13.72 paste-perf — huge-text wrap bypass.
    //
    // Pasting a long block (e.g. a 100KB structured prompt) without a
    // working bracketed-paste sequence delivers the entire string as
    // ONE text token. The composer's onChange handler synchronously
    // collapses it into a placeholder before the next React render,
    // but in the same tick this Cursor/MeasuredText pair still gets
    // constructed for the long content. If anything calls
    // `wrappedLines` (e.g. an intermediate render, a navigation
    // call), `wrapAnsi(100KB, columns)` runs once and blocks the
    // event loop for hundreds of ms — visible as a freeze on paste.
    //
    // For bypass-eligible texts we skip wrapAnsi entirely and produce
    // one WrappedLine per logical (`\n`-delimited) line. Cursor
    // navigation still works at the logical-line granularity, which
    // is all the brief pre-collapse interval needs.
    //
    // 2026-05-08: lowered from 8KB to 2KB after dogfood reported
    // "less laggy than before, but still laggy" with 0.13.72's 8KB
    // threshold. Real-world structured prompts commonly fall in the
    // 2–8 KB band. Typed content rarely reaches 2KB before the user
    // submits or pastes (~400 typed chars/min × ~5min idle = 2KB
    // upper bound for legitimate typed-only state); paste-collapse
    // fires at 200 chars / 5 lines anyway, so the bypass effectively
    // only kicks in during the brief pre-collapse render.
    const HUGE_TEXT_BYPASS_THRESHOLD = 2 * 1024
    if (this.text.length > HUGE_TEXT_BYPASS_THRESHOLD) {
      const lines: WrappedLine[] = []
      let cursor = 0
      const segments = this.text.split('\n')
      for (let i = 0; i < segments.length; i++) {
        const seg = segments[i]!
        const isPrecededByNewline = i === 0 || cursor > 0
        const endsWithNewline = i < segments.length - 1
        lines.push(new WrappedLine(seg, cursor, isPrecededByNewline, endsWithNewline))
        cursor += seg.length + (endsWithNewline ? 1 : 0)
      }
      return lines
    }
