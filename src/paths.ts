...
export function getOwlcodaDirLabel(): string {
  return labelHomePath(getOwlcodaDir())
}

export function getOwlcodaConfigLabel(): string {
  return labelHomePath(getOwlcodaConfigPath())
}

function labelHomePath(path: string): string {
  const home = homedir()
...
export function getOwlcodaDaemonLogPath(): string {
  return join(getOwlcodaDir(), 'daemon.log')
}
