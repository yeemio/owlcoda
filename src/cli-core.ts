...
import { VERSION, BUILD_INFO, buildTag } from './version.js'
import {
...
export function parseArgs(argv: string[]): {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '-r':
      case '--endpoint':
        // `--endpoint` is the canonical endpoint-first form (ADR
        // docs/architecture/model-connection-layer-decision-2026-05-12.md).
        // `--router` / `-r` retained as legacy aliases — same destination.
        routerUrl = args[++i]
        break
      case '--model':
      case '-m':
        ...
        break
      case '--mode':
        mode = parseOperatingMode(args[++i]) ?? undefined
        break
      case '--daemon-only':
        daemonOnly = true
        break
      ...
      case '--auto-approve':
        ...
        break
      case '--allow-tool':
        appendCommaSeparatedValues(allowTools, args[++i])
        break
      case '--deny-tool':
...
function appendCommaSeparatedValues(out: string[], raw: string | undefined): void {
  if (!raw) return
...
export function printHelp(): void {
  console.error(`owlcoda v${VERSION} (${buildTag()}) — Local AI coding platform
...
Setup & diagnostics:
  ...
  owlcoda cutover-status [--gate X] [--json]   Gate cutover-readiness from telemetry
  owlcoda shadow-status [--json]   Routing + microcompact shadow distributions
...
Training data (opt-in):
  owlcoda training status                       Collected dataset statistics
  owlcoda training path                         Print collected.jsonl path
  owlcoda training report [--from-sessions]     Collected stats (default) | historical quality distribution
  owlcoda training export [jsonl] [--limit N] [--sanitize]
                                                Stream collected JSONL (default)
  owlcoda training export [fmt] --from-sessions Export jsonl|sharegpt|insights from history
  owlcoda training scan                         Batch-score historical sessions into collected dataset
...
export async function doStop(force = false): Promise<void> {
  ...
  if (pid === null) {
    ...
    // partial uninstall — the Windows symptom) by its /healthz PID and kill it.
    if (force) {
      const baseUrl = getBaseUrl(loadConfig())
      let stoppedPid: number | null
      try {
        stoppedPid = await forceStopOrphanDaemon(baseUrl)
      } catch (err) {
        const reason = err instanceof Error ? err.message : String(err)
        console.error(`Failed to force-stop orphan OwlCoda daemon on ${baseUrl}: ${reason}`)
        process.exit(1)
      }
      if (stoppedPid !== null) {
        removePid()
        removeRuntimeMeta()
        console.error(`Force-stopped orphan OwlCoda daemon (PID ${stoppedPid}) on ${baseUrl}.`)
        return
      }
      console.error('owlcoda is not running (no PID file, and no live OwlCoda daemon answering on the configured port).')
      process.exit(1)
    }
    console.error('owlcoda is not running (no PID file). If a daemon is orphaned, run `owlcoda stop --force`.')
    process.exit(1)
  }
  if (!isPidAlive(pid)) {
...
// NO_COLOR / non-TTY (avoid leaking escape codes into pipes).
...
/**
 * Compare daemon's reported version vs the CLI binary's VERSION. When they
 * differ, print a dim hint suggesting `owlcoda stop && owlcoda` to align —
 * we do NOT auto-restart because the user likely has in-flight sessions.
 *
 * Returns true if a mismatch was reported (used by tests).
 */
export function warnIfDaemonStale(
  ...
  const action = activeClientCount !== undefined && activeClientCount > 0
    ? `Close active clients, or run \`owlcoda stop --force && owlcoda\` to align.`
    : `Run \`owlcoda stop && owlcoda\` to align.`
  console.error(dim(
    `  ⚠ Daemon running v${daemonVersion}, CLI is v${cliVersion}. ${action}`,
  ))
  return true
}
...
export async function doStatus(): Promise<void> {
  ...
  const activeClients = getActiveLiveClientsForRuntime(meta)
  warnIfDaemonStale(meta.version, VERSION, activeClients.length)
...
/**
 * Bare-startup escape hatch. When the selected/default model needs the local
 * runtime and preflight can't proceed, surface the configured direct-endpoint
 * (cloud) models that route WITHOUT the local runtime — and how to start one —
 * instead of silently shunting to Admin. Returns null when there is no such
 * alternative (genuine first-run / local-only configs keep the Admin-only flow).
 */
export function buildCloudFallbackHint(
  models: OwlCodaConfig['models'] | undefined,
  selectedModelId: string,
): string | null {
  const direct = (models ?? []).filter(m => m.endpoint != null)
  if (direct.length === 0) return null
  const first = direct[0]!
  const firstName = first.aliases?.[0] ?? first.id
  return [
    `Selected model "${selectedModelId}" needs the local runtime, which isn't ready —`,
    `but ${direct.length} cloud model(s) route without it: ${direct.map(m => m.id).join(', ')}.`,
    `  • Use one now:          owlcoda -m ${firstName}`,
    '  • Or set it as default in Admin so plain `owlcoda` works.',
  ].join('\n')
}

export async function doService(
  passthroughArgs: string[],
  configPath?: string,
  port?: number,
  routerUrl?: string,
): Promise<void> {
  const sub = (passthroughArgs[0] ?? 'status').toLowerCase()
  const svc = await import('./service-launchd.js')

  if (sub === 'status') {
    const st = svc.getLaunchdServiceStatus()
    console.error(`owlcoda service — launchd (${svc.SERVICE_LABEL})`)
    console.error(`  supported : ${st.supported ? 'yes (macOS)' : `no (${process.platform})`}`)
    console.error(`  installed : ${st.installed ? `yes — ${st.plistPath}` : 'no'}`)
    if (st.launchctl) {
      const lines = st.launchctl
        .split('\n')
        .filter(l => /\b(state|pid|last exit status|program)\b/i.test(l))
        .slice(0, 6)
  ...
  if (!svc.isLaunchdSupported()) {
    console.error(`launchd service is unsupported on ${process.platform} — macOS only.`)
    console.error(`(Linux systemd / Windows service are out of scope this round.)`)
    process.exit(1)
  }

  if (sub === 'install') {
    const { buildDaemonArgs } = await import('./daemon.js')
    const { getOwlcodaDaemonLogPath } = await import('./paths.js')
    const { randomUUID } = await import('node:crypto')
    const logPath = getOwlcodaDaemonLogPath()
    const programArgs = [process.execPath, ...buildDaemonArgs(configPath, port, routerUrl)]
    const res = svc.installLaunchdService({
      programArgs,
      stdoutPath: logPath,
      stderrPath: logPath,
    ...
    console.error(res.loaded
      ? `Loaded and running with KeepAlive.${res.note ? ' ' + res.note : ''}`
      : `Plist written but not loaded.${res.note ? ' ' + res.note : ''}`)
    console.error(`Check: owlcoda service status   ·   logs: tail -f ${logPath}`)
    return
  }

  if (sub === 'uninstall') {
    const res = svc.uninstallLaunchdService()
    console.error(`Uninstalled launchd service (${res.note}) → ${res.plistPath}`)
    return
  }

  console.error(`Unknown: owlcoda service ${sub}. Use: install | uninstall | status`)
  process.exit(1)
}
...
export async function doLaunch(
  ...
  if (!quietStartup) {
    ...
    console.error(`   Config: ${getOwlcodaConfigLabel()} (${modelCount} configured model${modelCount === 1 ? '' : 's'}; local user profile, not bundled)`)
    if (defaultModel) {
  ...
  // First-run cloud-first: when the user has no usable interactive model
  // (fresh init with no local backend, or only the legacy placeholder), skip
  // local-runtime preflight entirely and hand straight to Admin/onboarding.
  // Otherwise the user gets a misleading "Local runtime not reachable" error
  // even when they only ever wanted to add a cloud provider.
  const noUsableModel = !model && !defaultModel && !selectedModelConf
  if (noUsableModel) {
    console.error('No usable model is configured yet — configure a local runtime or cloud provider in OwlCoda Admin to continue.')
    console.error('First-run order: `owlcoda init` (create config) → `owlcoda admin` (add a model) → `owlcoda` (start the REPL).')
  ...
  if (!isDirectEndpointModel) {
    ...
    if (!preflight.canProceed) {
      ...
      const cloudHint = buildCloudFallbackHint(config.models, selectedModel)
      if (cloudHint) {
        console.error(cloudHint)
        console.error('')
      }
  ...
  // Surface a dim hint on launch when the running daemon predates the CLI
  // binary the user is invoking — common after `npm i -g` upgrades that
  // leave the long-running daemon untouched.
  if (!quietStartup) {
    warnIfDaemonStale(runtimeMeta.version)
  }
  ...
  try {
    await deps.startNativeRepl({
      ...
      apiKey,
      model: selectedModel,
      mode: resolveInitialMode({ flagMode: mode, settingsMode: config.mode }),
      // 0.13.65: was hard-coded 4096; let the resolver (which honors
      // OWLCODA_MAX_OUTPUT_TOKENS) pick. Default is now 32_768.
      maxTokens: resolveDefaultMaxOutputTokens(),
...
export async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'version': {
      ...
      console.error(`  build: ${buildTag()} · built ${BUILD_INFO.builtAt}`)
    ...
    case 'cutover-status': {
      const { buildAllCutoverStatuses, formatCutoverStatus } = await import('./native/cutover-status.js')
      const gate = passthroughArgs.includes('--gate')
        ? passthroughArgs[passthroughArgs.indexOf('--gate') + 1]
        : undefined
      const { statuses, filesRead } = buildAllCutoverStatuses(gate ? { gate } : {})
      if (jsonOutput) {
        console.log(JSON.stringify({ statuses, filesRead }, null, 2))
      } else if (statuses.length === 0) {
        console.error(gate ? `No cutover gate named "${gate}".` : 'No cutover gates configured.')
      } else {
        console.error(statuses.map(formatCutoverStatus).join('\n\n'))
        console.error(`\n(${filesRead.length} ledger file(s) read from ~/.owlcoda/telemetry/)`)
      }
      process.exit(0)
    }
    case 'shadow-status': {
      const { buildShadowStatus, formatShadowStatus } = await import('./native/shadow-status.js')
      const status = buildShadowStatus()
      if (jsonOutput) {
        console.log(JSON.stringify(status, null, 2))
      } else {
        console.error(formatShadowStatus(status))
      }
      process.exit(0)
    }
    case 'ui': {
    ...
    case 'sessions': {
      ...
      const tag = passthroughArgs.includes('--tag')
        ...
        : undefined
      const includeTest = passthroughArgs.includes('--include-test')

      let sessions = listSessions()
      if (!includeTest) {
        // Filter out test fixtures (test-model, test-*) so user-facing list
        // isn't polluted by leftover sessions from automated tests.
        sessions = sessions.filter(s => {
          const m = (s.model ?? '').toString()
          return !(m === 'test-model' || /^test[-_]/i.test(m))
        })
      }
      if (tag) {
    ...
    case 'skills': {
      ...
      if (subCmd === 'info') {
        ...
        if (passthroughArgs.includes('--json')) {
          ...
          console.error(`\n  Commands:`)
          console.error(`    owlcoda skills list                   List learned + third-party skills`)
          console.error(`    owlcoda skills add <url> --ref <ref>  Install third-party skill`)
          console.error(`    owlcoda skills remove <id>            Remove third-party skill`)
      ...
      if (subCmd === 'list' || subCmd === undefined) {
        ...
        const { listThirdPartySkills } = await import('./skills/third-party.js')
        const skills = await listSkills()
        const thirdParty = listThirdPartySkills(process.cwd())
        if (skills.length === 0 && thirdParty.length === 0) {
          console.error('No learned skills yet.')
          console.error('Use "owlcoda skills synth <session-id>" to synthesize from a session.')
          process.exit(0)
        }
        if (jsonOutput) {
          process.stdout.write(JSON.stringify({ learned: skills, thirdParty }, null, 2) + '\n')
        } else {
          if (skills.length > 0) {
          ...
          if (thirdParty.length > 0) {
            console.error(`Third-party Skills (${thirdParty.length}):`)
            for (const e of thirdParty) {
              console.error(`  ${e.id}  ${e.origin}  ${e.source}@${e.pinnedRef}  ${e.integrity}`)
            }
          }
        }
        process.exit(0)
      ...
      if (subCmd === 'import') {
        if (!skillArg) { console.error('Usage: owlcoda skills import <file.json|SKILL.md|skill-dir> [--overwrite]'); process.exit(1) }
        const { importSkillsFromPath } = await import('./skills/store.js')
        const { invalidateSkillIndex } = await import('./skills/injection.js')
        const overwrite = passthroughArgs.includes('--overwrite')
        const result = await importSkillsFromPath(skillArg, overwrite)
        invalidateSkillIndex()
      ...
      if (subCmd === 'add') {
        const url = passthroughArgs[1] ?? ''
        if (!url) { console.error('Usage: owlcoda skills add <git-url> --ref <sha|tag> [--skill <subpath>] [--id <id>] [--force]'); process.exit(1) }
        const refIdx = passthroughArgs.indexOf('--ref')
        const ref = refIdx !== -1 ? (passthroughArgs[refIdx + 1] ?? '') : ''
        if (!ref) { console.error('Error: --ref <sha|tag> is required'); process.exit(1) }
        const skillIdx = passthroughArgs.indexOf('--skill')
        const skill = skillIdx !== -1 ? (passthroughArgs[skillIdx + 1] ?? undefined) : undefined
        const idIdx = passthroughArgs.indexOf('--id')
        const id = idIdx !== -1 ? (passthroughArgs[idIdx + 1] ?? undefined) : undefined
        const force = passthroughArgs.includes('--force')
        const { addThirdPartySkill } = await import('./skills/third-party.js')
        const res = await addThirdPartySkill({ projectRoot: process.cwd(), url, ref, skill, id, force })
        for (const w of res.warnings) console.error(`Warning: ${w}`)
        if (!res.ok) {
          console.error(`Install refused: ${res.reason}`)
          process.exit(1)
        }
        console.error(`Installed third-party skill "${res.id}" (installed and injectable; not configured).`)
        process.exit(0)
      }

      if (subCmd === 'remove') {
        if (!skillArg) { console.error('Usage: owlcoda skills remove <id>'); process.exit(1) }
        const { removeThirdPartySkill } = await import('./skills/third-party.js')
        const removed = await removeThirdPartySkill(process.cwd(), skillArg)
        if (!removed) {
          console.error(`Third-party skill '${skillArg}' not found.`)
          process.exit(1)
        }
        console.error(`Removed third-party skill: ${skillArg}`)
        process.exit(0)
      }

      console.error(`Unknown skills subcommand: ${subCmd}`)
      console.error('Usage: owlcoda skills [list|show|synth|delete|export|import|stats|cleanup|search|match|add|remove] [<id>]')
      process.exit(1)
    }
    case 'training': {
      const { readFile, readdir, rm } = await import('node:fs/promises')
      const { join } = await import('node:path')
      const { getCollectedPath, getManifestPath, readCollectedStatus, readCollectedLines } =
        await import('./data/training-store.js')
      const subCmd = passthroughArgs[0] ?? 'status'
      // jsonOutput is the outer flag from parseArgs (parsed at the top level
      // before subcommand args). Don't re-derive it from passthroughArgs —
      // `--json` is consumed by the top-level switch and never reaches here.
      const fromSessions = passthroughArgs.includes('--from-sessions')
      ...
      switch (subCmd) {
        case 'status': {
          ...
          } else if (!status.manifestPresent && status.lineCount === 0) {
            console.log('\n📦 No training data collected yet.\n')
            console.log(`  Data will be auto-collected to ${status.path}`)
            console.log(`  when sessions end with quality >= 60\n`)
          } else {
            console.log(`\n📦 Training Data Status\n`)
            console.log(`  Collected:       ${status.totalCollected} sessions`)
            console.log(`  Skipped:         ${status.totalSkipped} sessions`)
            console.log(`  Average quality: ${status.averageQuality}/100`)
            console.log(`  Last collected:  ${status.lastCollectedAt || 'never'}`)
            console.log(`  File:            ${status.path}`)
            console.log(`  File size:       ${(status.fileSize / 1024).toFixed(1)} KB`)
            console.log(`  JSONL lines:     ${status.lineCount}`)
            console.log()
          }
          process.exit(0)
        }
        ...
        case 'report': {
          // Default mode: report on the actual collected dataset (manifest +
          // collected.jsonl). Old historical-session quality distribution
          // analysis is preserved behind --from-sessions.
          if (!fromSessions) {
            const status = await readCollectedStatus()
            if (jsonOutput) {
              process.stdout.write(JSON.stringify({ source: 'collected', ...status }, null, 2) + '\n')
            } else if (!status.manifestPresent && status.lineCount === 0) {
              console.log('\n📊 Training Data Report\n')
              console.log('  No collected dataset found.')
              console.log(`  Expected file:    ${status.path}`)
              console.log('  Tip: enable trainingCollection in config and finish a session,')
              console.log('       or use `owlcoda training scan` to backfill from history.')
              console.log('  For historical quality distribution: `owlcoda training report --from-sessions`\n')
            } else {
              console.log(`\n📊 Training Data Report (collected dataset)\n`)
              console.log(`  Collected:        ${status.totalCollected} sessions`)
              console.log(`  Skipped:          ${status.totalSkipped} sessions`)
              console.log(`  Average quality:  ${status.averageQuality}/100`)
              console.log(`  Last collected:   ${status.lastCollectedAt || 'never'}`)
              ...
              console.log(`  JSONL lines:      ${status.lineCount}`)
              console.log()
              console.log(`  Quality distribution from historical sessions: \`owlcoda training report --from-sessions\``)
              console.log()
            }
            process.exit(0)
          }
          ...
          if (jsonOutput) {
            process.stdout.write(JSON.stringify({ source: 'sessions', ...report }, null, 2) + '\n')
          } else {
        ...
        case 'export': {
          // Default mode: stream collected.jsonl through optional sanitizer.
          // Legacy historical-session export (jsonl/sharegpt/insights derived
          // from ~/.owlcoda/sessions/*.json) is preserved behind --from-sessions
          // because it produces formats the collector itself does not store.
          const exportFormat = (passthroughArgs[1] && !passthroughArgs[1].startsWith('--') ? passthroughArgs[1] : 'jsonl') as 'jsonl' | 'sharegpt' | 'insights'
          if (!['jsonl', 'sharegpt', 'insights'].includes(exportFormat)) {
            console.error(`Unknown format: ${exportFormat}`)
            console.error('Usage: owlcoda training export [jsonl|sharegpt|insights] [--limit N] [--sanitize] [--from-sessions]')
            process.exit(1)
          }
          const sanitize = passthroughArgs.includes('--sanitize')
          const limitArg = passthroughArgs.includes('--limit')
            ? parseInt(passthroughArgs[passthroughArgs.indexOf('--limit') + 1] || '0', 10)
            : undefined

          if (!fromSessions) {
            // Collected dataset only stores the JSONL `{"messages":[...]}`
            // shape produced by sessionToJsonl(). sharegpt/insights cannot be
            // synthesized from that without re-loading the source session, so
            // require --from-sessions for those formats.
            if (exportFormat !== 'jsonl') {
              console.error(`Format '${exportFormat}' is only available against historical sessions.`)
              console.error(`Run: owlcoda training export ${exportFormat} --from-sessions [--limit N]`)
              process.exit(1)
            }
            const status = await readCollectedStatus()
            if (status.lineCount === 0) {
              console.error('No collected training data found.')
              console.error(`Expected: ${status.path}`)
              console.error('Use `owlcoda training scan` to backfill, or `--from-sessions` to export from history.')
              process.exit(1)
            }
            let written = 0
            for await (const line of readCollectedLines({ limit: limitArg, sanitize })) {
              process.stdout.write(line + '\n')
              written++
            }
            console.error(`\nExported ${written} JSONL lines from collected dataset (${status.path})`)
            process.exit(0)
          }
          ...
          console.error(`Exporting training data from sessions (format: ${exportFormat})...`)
        ...
        default:
          ...
          console.error('Usage: owlcoda training [status|scan|report|export|clear|path] [--json] [--from-sessions]')
          process.exit(1)
      }
      break
    }
    ...
    case 'launch': {
      if (dryRun) {
        ...
        await doLaunch(configPath, port, routerUrl, resumeSession, model, undefined, mode)
      }
      break
    }
  }
