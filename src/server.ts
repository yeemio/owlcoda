...
import { handleAdmissionStatus } from './endpoints/admission.js'
...
/**
 * Run a request handler with crash isolation. If it throws synchronously, the
 * error is contained to a 500 for THAT request and never propagates to the
 * process — without this, a throw in the dispatch path (parse / routing /
 * requestId) reaches `uncaughtException` → `shutdown()`, taking down the shared
 * daemon for every client. `onError` lets the caller log (now captured in
 * daemon.log). Async route rejections are handled by each route's own `.catch`
 * (and `unhandledRejection`, which only logs); this guards the synchronous gap.
 */
export function dispatchRequestSafely(
  handler: () => void,
  ...
  try {
    ...
    try { onError?.(err) } catch { /* logging must never escalate */ }
    try {
      if (!res.headersSent) {
      ...
      /* response already gone — nothing more to do, but never rethrow */
    }
  }
}
...
function handleRequest(
  ...
  recordRequestStart()
  res.on('finish', () => {
    // This fires on a later tick — a throw here would also reach
    // uncaughtException, so keep telemetry failures from escalating.
    try {
  ...
  if (method === 'GET' && url === '/v1/api-info') {
    ...
    sendJson(res, 200, {
      ...
      endpoints: [
        ...
        { method: 'GET', path: '/v1/admission', description: 'Cross-process admission gate state: per-upstream effective limit, in-flight, queue, admit/reject/rate-limit counts' },
  ...
  if (method === 'POST' && url === '/v1/structured-output') {
  ...
  if (method === 'GET' && url === '/v1/admission') {
    handleAdmissionStatus(req, res)
    return
  }
...
export function startServer(config: OwlCodaConfig): http.Server {
  ...
  const server = http.createServer((req, res) => {
    dispatchRequestSafely(
      () => handleRequest(req, res, config, modelTruth, runtimeAdminDeps, adminApiDeps),
      res,
      (err) => console.error(`[owlcoda] request handler crashed (isolated, request 500'd): ${(err as Error)?.stack ?? String(err)}`),
    )
  })
