...
export class StreamTranslator {
  ...
  private upstreamUsage: { prompt_tokens: number; completion_tokens: number; cached_tokens: number } | null = null
  ...
  /**
   * Usage object for the final message_delta event. message_start can only
   * carry an input ESTIMATE (the real upstream usage arrives at stream end), so
   * the final message_delta is the only place a streaming client can learn the
   * true input_tokens and prefix-cache hit. When real upstream usage is present
   * (e.g. owlmlx / OpenAI-compatible servers that report prompt_tokens_details),
   * surface input/cache_read (kept disjoint, matching the Anthropic shape) so a
   * downstream consumer of owlcoda's SSE sees local-backend cache hits — not
   * just owlcoda's internal /cost. With no upstream usage, stay output-only (no
   * regression: the estimate already went out in message_start).
   */
  private finalDeltaUsage(): Record<string, number> {
    if (!this.upstreamUsage) {
      return { output_tokens: this.estimatedOutputTokens }
    }
    const u = this.getFinalUsage()
    return {
  ...
  getFinalUsage(): { inputTokens: number; outputTokens: number; cacheReadTokens: number } {
    // OpenAI prompt_tokens includes cached prefix; keep input/cache_read
    // disjoint to match the Anthropic usage shape (see translate/response.ts).
    if (this.upstreamUsage) {
      const cached = this.upstreamUsage.cached_tokens
      return {
        inputTokens: Math.max(0, this.upstreamUsage.prompt_tokens - cached),
        outputTokens: this.estimatedOutputTokens,
        cacheReadTokens: cached,
      }
    }
    return {
      inputTokens: this.inputTokenEstimate,
      outputTokens: this.estimatedOutputTokens,
      cacheReadTokens: 0,
    }
  }
