...
import { buildTelemetryEventEnvelope, recordTelemetryEnvelope } from './native/telemetry-envelope.js'
...
/**
 * Check router health by probing /healthz. Thin wrapper over the pure
 * computation that also emits one behavior-neutral runtime-health telemetry
 * envelope — the shared spine's first live producer. The sink is fire-and-forget
 * (off via OWLCODA_TELEMETRY_EVENTS=0); the returned ServiceCheck is unchanged.
 */
export async function checkRouterHealth(routerUrl: string, config?: OwlCodaConfig): Promise<ServiceCheck> {
  const check = await computeRouterHealth(routerUrl, config)
  emitRouterHealthTelemetry(check)
  return check
}

function emitRouterHealthTelemetry(check: ServiceCheck): void {
  const decision = check.status === 'healthy_reused' ? 'pass' : check.status === 'blocked' ? 'block' : 'warn'
  const severity = check.status === 'healthy_reused' ? 'info' : 'warn'
  recordTelemetryEnvelope(buildTelemetryEventEnvelope({
    ...
    severity,
    decision,
    reasonCode: check.status,
    ...(check.responseTimeMs !== undefined ? { attributes: { responseTimeMs: check.responseTimeMs } } : {}),
  }))
}

async function computeRouterHealth(routerUrl: string, config?: OwlCodaConfig): Promise<ServiceCheck> {
...
export async function checkBackendModel(
  ...
  if (found) {
    return {
      ...
      url: routerUrl,
      status: 'healthy_reused',
      detail: `Model available via ${runtimeProbe.source === 'deprecated_router_models' ? 'openai_compatible_models (legacy visibility surface)' : runtimeProbe.source}`,
    }
  }
...
async function runPreflightWithTruth(
  ...
  const router: ServiceCheck = hasLocalModels
    ? buildRouterCheckFromSnapshot(snapshot, config.routerUrl, unresolvedProtocol)
    : {
        name: 'Local runtime',
        url: config.routerUrl,
        status: 'healthy_reused',
        detail: 'Skipped — no configured models require a local runtime',
      }
...
function buildRouterCheckFromSnapshot(
  ...
  // Display label: collapse the legacy `deprecated_router_models` probe enum
  // to the engineering reality (OpenAI-compatible /v1/models). The legacy
  // visibility surface is informational only, not a user-actionable warning.
  const displayLabel = sourceLabel === 'deprecated_router_models'
  ...
  const detail = sourceLabel === 'runtime_status'
    ...
    || sourceLabel === 'healthz'
    ? `Healthy via ${displayLabel}${snapshot.platformVisibility?.deprecatedFallback ? ' (legacy visibility surface)' : ''} (${snapshot.runtimeProbeDetail})`
    : `Healthy via ${displayLabel}`
