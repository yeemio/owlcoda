...
export function normalizeProviderKind(model: Pick<ConfiguredModel, 'endpoint' | 'headers'> & { provider?: string }): ProviderKind {
  ...
  if (explicit === 'anthropic' || explicit === 'claude' || explicit === 'deepseek' || explicit === 'glm') return 'anthropic'
  if (explicit === 'openrouter') return 'openai-compat'
  if (explicit === 'bailian') return 'openai-compat'
  if (
    explicit === 'openai-compat'
    || explicit === 'openai'
    || explicit === 'openai_compat'
    || explicit === 'gpt'
    || explicit === 'gemini'
    || explicit === 'grok'
    || explicit === 'xai'
  ...
  if (explicit === 'minimax-anthropic' || explicit === 'minimax-anthropic-cn' || explicit === 'minimax') return 'anthropic'
  ...
  if (endpoint.includes('generativelanguage.googleapis.com')) return 'openai-compat'
  if (endpoint.includes('api.x.ai')) return 'openai-compat'
