...
import { resolveSustainedWorkCapability } from './model-capabilities.js'
...
function scoreModel(
  ...
  const sustainedWork = resolveSustainedWorkCapability(model)
  ...
  if (intent === 'code' || intent === 'analysis') {
    if (!sustainedWork.longTaskRecommendationEligible) {
      score -= 80
      reasons.push('not signed off for sustained work')
    }
