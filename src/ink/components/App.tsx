...
import { traceInputLatencyInputEvent, traceInputLatencyParsedKeys, traceInputLatencyStdinChunk } from '../input-latency-trace.js';
...
import { isXtermJs, setXtversionName, supportsExtendedKeys, usesLowChurnTerminalMode } from '../terminal.js';
import { getTerminalFocused, setTerminalFocused } from '../terminal-focus-state.js';
import { TerminalQuerier, xtversion, cursorPosition } from '../terminal-querier.js';
import {
...
import { CURSOR_HOME, DISABLE_KITTY_KEYBOARD, DISABLE_MODIFY_OTHER_KEYS, ENABLE_KITTY_KEYBOARD, ENABLE_MODIFY_OTHER_KEYS, FOCUS_IN, FOCUS_OUT } from '../termio/csi.js';
import { DBP, DFE, DISABLE_MOUSE_TRACKING, EBP, EFE, ENTER_ALT_SCREEN, EXIT_ALT_SCREEN, HIDE_CURSOR, SHOW_CURSOR } from '../termio/dec.js';
...
export default class App extends PureComponent<Props, State> {
  ...
  override componentDidMount() {
    ...
    if (this.props.stdout.isTTY && !isEnvTruthy(process.env.OWLCODA_ACCESSIBILITY)) {
  ...
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (isEnabled) {
      ...
      if (this.rawModeEnabledCount === 0) {
        ...
        // OwlCoda low-churn mode skips optional terminal side channels so
        // plain typing stays on the shortest possible input/render path.
        if (!usesLowChurnTerminalMode()) {
        ...
        if (!usesLowChurnTerminalMode()) {
          setImmediate(() => {
            ...
            // Ambiguous-width probe: East-Asian "Ambiguous" glyphs (→ ≤ ⚠ │
            // ● …) render narrow in Western terminals but WIDE in CJK-locale
            // terminals. owlcoda's width math assumes narrow; on a wide
            // terminal that desyncs the virtual screen → truncated/duplicated
            // rows. Measure the real cursor advance and flip stringWidth to
            // match. Fire-and-forget; bounded by the same DA1 sentinel.
            void this.probeAmbiguousWidth();
          });
        } else {
          this.applyAmbiguousWidthFallback('skipping probe in low-churn terminal mode');
        }
      }
  ...
  // Detect whether the terminal renders East-Asian Ambiguous glyphs wide
  // (CJK locale) or narrow (Western), and flip stringWidth to match. The probe
  // briefly switches to the alternate screen as a scratch buffer, writes one
  // Ambiguous glyph at column 1, reads the cursor advance via DECXCPR
  // (CSI ?6n), then returns to the main screen. This avoids writing/clearing
  // the current main-screen row after the first render has already painted.
  // Any failure (no DSR reply, garbled delta, write error) falls back to CJK
  // locale metadata before keeping the narrow default. Bounded by the querier's
  // DA1 sentinel — never hangs.
  private applyAmbiguousWidthFallback(reason: string): boolean {
    const applied = applyAmbiguousWidthLocaleFallback();
    const mode = isAmbiguousWidthWide() ? 'WIDE (2)' : 'narrow (1)';
    logForDebugging(
      `ambiguous-width: ${reason} — ${
        applied
          ? `using CJK locale fallback ${mode}`
          : `keeping current mode ${mode}`
      }`,
    );
    return applied;
  }

  probeAmbiguousWidth = async (): Promise<void> => {
    const stdout = this.props.stdout;
    if (!stdout.isTTY) {
      this.applyAmbiguousWidthFallback('not a TTY');
      return;
    }
    if (this.props.altScreenActive) {
      this.applyAmbiguousWidthFallback('skipping probe while alternate screen is active');
      return;
    }
    try {
      // Use a scratch alternate-screen buffer. Probing on the main screen
      // would require erasing/repainting the current physical row, which is
      // exactly the rendering surface this detector is meant to protect.
      stdout.write(ENTER_ALT_SCREEN + CURSOR_HOME + HIDE_CURSOR);
      const beforeP = this.querier.send(cursorPosition());
      stdout.write('→'); // one East-Asian Ambiguous glyph (→ U+2192)
      const afterP = this.querier.send(cursorPosition());
      const [before, after] = await Promise.all([
        beforeP,
        afterP,
        this.querier.flush(),
      ]);
      // Return to the main screen + restore cursor visibility. We do not erase
      // the scratch glyph: exiting the alternate buffer discards it.
      stdout.write(EXIT_ALT_SCREEN + SHOW_CURSOR);
      if (!before || !after) {
        this.applyAmbiguousWidthFallback('no DSR reply');
        return;
      }
      const wide = interpretAmbiguousWidthProbe(before.col, after.col);
      if (wide === null) {
        this.applyAmbiguousWidthFallback(
          `implausible advance ${before.col}->${after.col}`,
        );
        return;
      }
      setAmbiguousWidthWide(wide);
      logForDebugging(
        `ambiguous-width: terminal renders Ambiguous glyphs ${wide ? 'WIDE (2)' : 'narrow (1)'}`,
      );
    } catch (err) {
      // Best-effort restore; a probe failure must never affect startup.
      try {
        stdout.write(EXIT_ALT_SCREEN + SHOW_CURSOR);
      } catch {
        /* ignore */
  ...
  processInput = (input: string | Buffer | null, inputSeq: number | null = null): void => {
    // Parse input using our state machine
    const [keys, newState] = parseMultipleKeypresses(this.keyParseState, input);
    this.keyParseState = newState;
    traceInputLatencyParsedKeys(inputSeq, keys);
  ...
  handleReadable = (): void => {
    ...
    try {
      ...
      while ((chunk = this.props.stdin.read() as string | null) !== null) {
        ...
        this.processInput(chunk, traceInputLatencyStdinChunk(chunk));
      }
    } catch (error) {
  ...
  handleSuspend = (): void => {
    ...
    // Emit suspend event for the host app to handle. Mostly just has a notification
    ...
    const resumeHandler = () => {
      ...
      if (this.props.stdout.isTTY) {
        if (!isEnvTruthy(process.env.OWLCODA_ACCESSIBILITY)) {
          this.props.stdout.write(HIDE_CURSOR);
        }
