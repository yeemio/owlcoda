  ...
  render(
    ...
    if (next.staticCommit && next.staticCommit.rowCount > 0) {
      ...
      const commitPatches: Diff = [
        ...
        { type: 'stdout', content: '\n'.repeat(next.staticCommit.rowCount) },// scroll pre-commit viewport down
      ]
      // Force full-reset of dynamic region. The clearTerminal patch uses
      // getClearTerminalSequence() from Task A — scrollback-preserving,
      // so the rows we just committed stay readable in terminal scrollback.
      const resetPatches = fullResetSequence_CAUSES_FLICKER(
        next,
        'clear',
        stylePool,
      )
    ...
    if (this.framesSinceFullReset >= LogUpdate.SCRUB_EVERY_N_FRAMES) {
      this.framesSinceFullReset = 0
      this.state.previousOutput = ''
      return fullResetSequence_CAUSES_FLICKER(next, 'periodic_scrub', stylePool)
    }
