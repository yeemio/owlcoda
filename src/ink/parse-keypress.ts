  ...
  for (const token of tokens) {
    if (token.type === 'sequence') {
      ...
      if (inPaste) {
        ...
        // the continuation `[<btn;col;rowM` (or, in the ESC[` split case,
        // plain `<btn;col;rowM`) arrived as text. Re-synthesize the missing
        // prefix so the scroll event still fires instead of leaking into the
        // prompt. The spurious Escape is gone; App.tsx's readableLength check
        // prevents it. The X10 Cb slot is narrowed to the wheel range
        // [\x60-\x7f] (0x40|modifiers + 32) — a full [\x20-] range would
        // match typed input like `[MAX]` batched into one read and silently
        // drop it as a phantom click. Click/drag orphans leak as visible
        // garbage instead; deletable garbage beats silent loss.
        const resynthesized = token.value.startsWith('<')
          ? '\x1b[' + token.value
          : '\x1b' + token.value
