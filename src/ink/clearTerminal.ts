...
/**
 * Default full-reset sequence. Preserves terminal scrollback so that
 * previously-committed Static rows (via watermark v2) remain visible
 * when the user scrolls up in Terminal.app / tmux.
 *
 * Used by Ink internals when recomposing from scratch (resize, full
 * repaint after a static commit, etc.).
 */
...
/**
 * Nuclear clear — wipes both visible screen AND terminal scrollback.
 * Reserved for user-invoked clears (`/clear` slash command). Not used
 * by Ink internals; only exposed for callers that explicitly want to
 * obliterate history.
 */
export function getNuclearClearSequence(): string {
  if (process.platform === 'win32') {
    if (isModernWindowsTerminal()) {
 ...
 */
export const clearTerminal = getNuclearClearSequence()
