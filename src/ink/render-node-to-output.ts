  ...
  if (yogaNode) {
    ...
    const positionChanged =
      ...
      (cached.x !== x ||
        ...
        cached.width !== width ||
        cached.height !== height)
    // A leaf text node's width changes on every keystroke. That is local
    // damage, not a structural layout shift, and forcing full-screen damage
    // here makes mediated terminals such as cmux feel sluggish while typing.
    // Real structural shifts are position/height changes; horizontal sibling
    // movement caused by a width change is caught on the shifted sibling's x.
    if (
      cached !== undefined &&
      (cached.x !== x || cached.y !== y || cached.height !== height)
    ) {
      layoutShifted = true
    }
