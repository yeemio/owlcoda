...
import { InkInstanceContext, type InkInstanceHandle } from './components/InkInstanceContext.js';
...
import { SYNC_OUTPUT_SUPPORTED, supportsExtendedKeys, type Terminal, usesLowChurnTerminalMode, usesSafeTerminalRepaintMode, writeDiffToTerminal } from './terminal.js';
...
import { isInputLatencyTraceEnabled, traceInputLatencyCheckpoint, traceInputLatencyWriteComplete } from './input-latency-trace.js';
...
export default class Ink {
  ...
  // One-shot flag: forceRedraw() sets it, onRender() reads + clears it on
  // the very next frame. Forces that frame's log.render call to take the
  // forceFullRepaint=true path → safeFullRepaint(). This replaces the legacy
  // forceRedraw stdout.write(ERASE_SCREEN + CURSOR_HOME) with a fully
  // patch-system-mediated repaint so the main-screen \x1b[2J invariant holds
  // (no path may emit ED 2 to main except /clear and alt-screen lifecycle).
  private pendingForceFullRepaint = false;
  ...
  // Append-only queue of scrollback commit lines. Drained by onRender
  // into frame.staticCommit before log.render; cleared after each drain.
  // React-side <Static> components append via enqueueStaticCommit.
  // NEVER written to stdout from here — log.render owns emission.
  private pendingStaticCommit: string[] = [];
  ...
  constructor(private readonly options: Options) {
    ...
    const deferredRender = (): void => {
      if (isInputLatencyTraceEnabled()) {
        traceInputLatencyCheckpoint('render-throttle-fired');
      }
      queueMicrotask(() => {
        if (isInputLatencyTraceEnabled()) {
          traceInputLatencyCheckpoint('render-microtask-start');
        }
        this.onRender();
      });
    };
  ...
  exitAlternateScreen(): void {
    ...
    // re-enable mouse (skip if OWLCODA_DISABLE_MOUSE)
    ...
    if (this.altScreenActive) {
      ...
      this.repaint();
      // repaint() alone leaves the next main-screen frame eligible for the
      // blit fast path even though the backing frame is blank. After an
      // external alt-screen handoff that can swallow composer chrome and
      // other unchanged rows on return. Force the next render down the
      // full-damage path.
  ...
  onRender() {
    ...
    // Watermark-v2: drain pending scrollback commits into frame.staticCommit
    // for this render cycle. log.render (Task B) prepends commit ANSI to
    // its returned Diff when this field is set with rowCount > 0.
    //
    // CRITICAL: rowCount must count PHYSICAL terminal rows, not logical
    // lines. A long path like `read(/Users/.../very_long_file.md)` is one
    // logical line but wraps to 2-3 terminal rows in an 80-col terminal.
    // Counting logical lines here under-scrolls the viewport, leaving the
    // wrapped tail of the commit text inside the dynamic viewport where
    // the ERASE_SCREEN in fullResetSequence then wipes it. Simultaneously,
    // the cursor-position accounting in the `cursor→(1,1) write→\x1b[K`
    // sequence goes out of sync with the terminal's auto-wrap, producing
    // the characteristic "row A tail + row B head on one scrollback row"
    // smear the user reported (e.g. `..._PROMPT_CN.md)gitrep/SIDEC`).
    //
    // Pre-wrap each logical line to viewport.width so the write stream's
    // explicit `\n`s match the terminal's physical row advance 1:1 — no
    // auto-wrap, no miscounting.
    if (this.pendingStaticCommit.length > 0) {
      const wrapWidth = Math.max(1, frame.viewport.width)
      const wrappedLines: string[] = [];
      for (const line of this.pendingStaticCommit) {
        // `line` itself may contain embedded \n (multi-line tool output).
        // Split first, then wrap each sub-line so each element of
        // wrappedLines is exactly one physical terminal row.
        for (const sub of line.split('\n')) {
          const wrapped = wrapAnsi(sub, wrapWidth, { trim: false, hard: true });
          // wrapAnsi returns a single string with \n at wrap points; split
          // back so every element is one physical row.
      ...
      const rowCount = wrappedLines.length;
      // Frame is readonly — construct a new object rather than mutating
      frame = { ...frame, staticCommit: { text, rowCount } };
    ...
    // forceFullRepaint = (a) one-shot flag set by forceRedraw() — consumed
    // here so the next frame returns to incremental cell-diff; OR (b) the
    // OWLCODA_RENDER_MODE=safe diagnostic opt-in. Both routes terminate at
    // the same safeFullRepaint() entry inside log.render — no parallel
    // full-repaint pipelines.
    const consumePendingFullRepaint = this.pendingForceFullRepaint;
    if (consumePendingFullRepaint) this.pendingForceFullRepaint = false;
    const forceFullRepaint = !this.altScreenActive && (consumePendingFullRepaint || usesSafeTerminalRepaintMode());
    ...
    SYNC_OUTPUT_SUPPORTED, forceFullRepaint);
    ...
    if (isInputLatencyTraceEnabled()) {
      ...
      const traceYogaVisited = this.lastYogaCounters.visited;
      const traceYogaMeasured = this.lastYogaCounters.measured;
      writeDiffToTerminal(this.terminal, optimized, !SYNC_OUTPUT_SUPPORTED, ({
        bytes
      }) => {
        traceInputLatencyWriteComplete({
          bytes,
          patches: diff.length,
          optimizedPatches: optimized.length,
          frameDurationMs: performance.now() - renderStart,
          writeCallbackMs: Number(process.hrtime.bigint() - writeStartNs) / 1_000_000,
          altScreen: this.altScreenActive,
          targetMoved,
          yogaVisited: traceYogaVisited,
          yogaMeasured: traceYogaMeasured
        });
      });
    } else {
      writeDiffToTerminal(this.terminal, optimized, !SYNC_OUTPUT_SUPPORTED);
  ...
  /**
   * Clear the visible viewport and force a full redraw.
   *
   * Public API used by ctrl+l (traditional readline-style refresh) and the
   * recovery path when the terminal was cleared externally (macOS Cmd+K)
   * and the cell-diff engine thinks unchanged cells don't need repainting.
   * Scrollback is preserved.
   *
   * Implementation note (2026-04-30 P0 收口): this used to write
   * `ERASE_SCREEN + CURSOR_HOME` directly via stdout.write before
   * scheduling a render. That bypassed the patch system and put a literal
   * \x1b[2J onto the main-screen byte stream, which on Terminal.app /
   * iTerm2 pushes the visible viewport into scrollback before erasing —
   * the same scrollback-pollution failure mode the safeFullRepaint refactor
   * was eliminating elsewhere. Now: set the one-shot pendingForceFullRepaint
   * flag, mark prev frame contaminated, and let onRender() route through
   * log.render → safeFullRepaint(). The visible repaint is identical
   * (clean viewport with current content), but no \x1b[2J leaks out.
   *
   * Alt-screen branch keeps `resetFramesForAltScreen()` (which uses
   * `reenterAltScreen` if needed) — alt-screen ED 2 is on the explicit
   * exception list of the main-screen invariant.
   */
  forceRedraw(): void {
    ...
    if (this.altScreenActive) {
      ...
      this.repaint();
      // repaint() resets frontFrame to 0×0. Without these flags the next
      // frame's blit copies from that empty screen and cell-diff would
      // produce no diff. onRender resets both flags at frame end.
      this.prevFrameContaminated = true;
  ...
  setAltScreenActive(active: boolean, mouseTracking = false): void {
    ...
    if (active) {
      ...
      this.repaint();
      // Leaving alt-screen also needs a one-shot full-damage render.
      // Without this the diff engine can trust the blank repaint() frame
      // and skip authoring-surface rows that were visually wiped.
      this.prevFrameContaminated = true;
    }
  }
   ...
   * render. Safe to call from React useLayoutEffect — this only
   * mutates an in-memory queue and schedules a re-render. Actual
   * stdout emission happens inside writeDiffToTerminal on the next
   * onRender cycle, so no race with the frame paint.
   */
  public enqueueStaticCommit(lines: readonly string[]): void {
    if (lines.length === 0) return;
    for (const line of lines) {
      this.pendingStaticCommit.push(line);
    }
    this.scheduleRender();
  }
  // Cached so its identity is stable across renders — context-change
  // detection is referential; a fresh object per render() would cascade
  // useInkInstance() consumer effects to re-run every frame.
  ...
  render(node: ReactNode): void {
    ...
    const tree = <App stdin={this.options.stdin} stdout={this.options.stdout} stderr={this.options.stderr} exitOnCtrlC={this.options.exitOnCtrlC} onExit={this.unmount} terminalColumns={this.terminalColumns} terminalRows={this.terminalRows} altScreenActive={this.altScreenActive} selection={this.selection} onSelectionChange={this.notifySelectionChange} onClickAt={this.dispatchClick} onHoverAt={this.dispatchHover} getHyperlinkAt={this.getHyperlinkAt} onOpenHyperlink={this.openHyperlink} onMultiClick={this.handleMultiClick} onSelectionDrag={this.handleSelectionDrag} onStdinResume={this.reassertTerminalModes} onCursorDeclaration={this.setCursorDeclaration} dispatchKeyboardEvent={this.dispatchKeyboardEvent}>
        <TerminalWriteProvider value={this.writeRaw}>
          <InkInstanceContext.Provider value={this.instanceHandle}>
            {node}
