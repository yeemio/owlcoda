// @ts-nocheck — React reconciler host config is inherently untyped
...
export function isDebugRepaintsEnabled(): boolean {
  if (debugRepaints === undefined) {
    debugRepaints = isEnvTruthy(process.env.OWLCODA_DEBUG_REPAINTS)
  }
...
const COMMIT_LOG = process.env.OWLCODA_COMMIT_LOG
let _commits = 0
let _lastLog = 0
let _lastCommitAt = 0
let _maxGapMs = 0
...
>({
  ...
  prepareForCommit: () => {
    if (isInputLatencyTraceEnabled()) traceInputLatencyCheckpoint('prepare-for-commit')
  ...
  resetAfterCommit(rootNode) {
    ...
    if (isInputLatencyTraceEnabled()) {
      traceInputLatencyCheckpoint('commit-after-layout', {
        commitMs: Number(_lastCommitMs.toFixed(3)),
      })
    }
    if (COMMIT_LOG) {
    ...
    if (isInputLatencyTraceEnabled()) traceInputLatencyCheckpoint('commit-schedule-render')
    rootNode.onRender?.()
    if (COMMIT_LOG) {
