...
- Respect the current runtime permission and network policy. If deployment network calls are blocked, stop and report the blocker; ask the user to run in an environment where they have explicitly allowed Render network access.
...
2) Add the MCP server with external coding-assistant (replace `<YOUR_API_KEY>`):
```bash
claude mcp add --transport http render https://mcp.render.com/mcp --header "Authorization: Bearer <YOUR_API_KEY>"
```

3) Restart external coding-assistant, then retry `list_services()`.
...
If deployment fails due to network issues (timeouts, DNS errors, connection resets), first distinguish provider/network failure from runtime policy blocking. When runtime policy blocks outbound deployment calls, do not weaken the policy from inside the skill; stop and ask the user to explicitly approve or run from an environment with Render network access.

Example guidance to the user:

```
The deploy needs Render network access, but the current runtime policy blocked it. Please approve network access or run this deploy from an environment where Render access is allowed.
```
