...
- If you see "screen capture checks are blocked in the sandbox", "could not create image from display", or Swift `ModuleCache` permission errors in a sandboxed run, stop and report the runtime policy blocker. Ask the user to approve screen capture or run from an environment where screen capture is allowed.
...
- If saving to the OS default location fails with permission errors in a sandbox, stop and report the runtime policy blocker. Ask the user to choose an allowed save path or approve the required filesystem access.
