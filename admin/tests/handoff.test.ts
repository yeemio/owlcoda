...
describe('pathToRoute', () => {
  it('maps canonical routes (from backend AdminHandoffRoute)', () => {
    // 'start' now redirects to 'models' — Start page removed.
    expect(pathToRoute('start')).toBe('models')
  ...
  it('defaults unknown paths to models', () => {
    expect(pathToRoute('')).toBe('models')
    expect(pathToRoute('something-else')).toBe('models')
  })

  it('retires the old runs route to models', () => {
    expect(pathToRoute('runs')).toBe('models')
    expect(pathToRoute('runs/latest')).toBe('models')
  })
})
...
describe('readHandoff', () => {
  it('returns defaults when URL is bare', () => {
    ...
    // Default landing is now models (Start page removed).
...
describe('buildHash', () => {
  it('emits canonical handoff paths (matches server buildAdminHandoffHash)', () => {
    // 'start' route maps to /models now — Start page removed.
    expect(buildHash({ route: 'start' })).toBe('#/models')
