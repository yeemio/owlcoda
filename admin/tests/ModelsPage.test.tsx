...
describe('ModelsPage (Phase β read-only)', () => {
  ...
  it('renders the local config source in overview', () => {
    const snap = { ...snapshot, configSourceLabel: '~/.owlcoda/config.json' }
    render(<ModelsPage snapshot={snap} onRefresh={() => {}} onSnapshotUpdate={() => {}} loading={false} />)
    expect(screen.getByTestId('overview-config-source')).toHaveTextContent('~/.owlcoda/config.json')
  })
  ...
  it('Shows credential source and recent endpoint health for endpoint-backed models', async () => {
    ...
    try {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('fact-endpointHealth')).toHaveTextContent('✓ 12ms (30s ago)')
      })
