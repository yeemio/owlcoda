    ...
    return (
      <section className="panel full" data-testid="orphans-empty">
        <PageHeader
          ...
          title={t('orphans')}
          sub={t('orphansEmptySub')}
          right={<Pill kind="ok">0 {t('discovered')}</Pill>}
        />
        <div className="empty">
          <span className="tone-ok">●</span> {t('noOrphanModels')}
        </div>
  ...
  return (
    <section className="panel full" data-testid="orphans-page">
      <PageHeader
        ...
        title={t('orphans')}
        sub={t('orphansSub')}
        right={<Pill kind="info" testId="orphans-discovered-count">{orphans.length} {t('discovered')}</Pill>}
      />
      ...
      <div className="batch-bar">
        ...
        <button type="button" onClick={() => toggleAll(false)} data-testid="orphans-select-none">{t('clear')}</button>
        <span className="tone-muted" data-testid="orphans-count">
          {t('selectedOf', { selected: selectedCount, total: orphans.length })}
        </span>
            ...
            >
              ...
              <div className="orphan-mode" role="tablist">
                <label className="orphan-mode-primary">
                  ...
                  />
                  <span><strong>{t('adoptNewConfig')}</strong></span>
                </label>
                <label className="orphan-mode-advanced">
                  ...
                  />
                  <span>{t('bindExistingLocal')} <span className="tone-subtle">{t('advancedRepoint')}</span></span>
                </label>
              </div>
              ...
              {isCreate ? (
                ...
                <div className="orphan-form">
                  <label className="field inline" style={{ minWidth: 260 }}>
                    <span className="field-label">{t('bindToLocalOnly')}</span>
                    <select
                      className="field-input"
                    ...
                    >
                      <option value="">{t('pickLocalConfig')}</option>
                  ...
                  {p.selected && p.targetModelId && (
                    <div className="tone-muted" data-testid={`orphan-bind-preview-${s.id}`} style={{ fontSize: 12 }}>
                      {t('willPatchBackend')} <code>{p.targetModelId}</code>.backendModel ← <code>{s.raw.discovered?.id ?? s.id}</code>
                    </div>
                  )}
      ...
      <div className="batch-bar">
        <span className="tone-muted" data-testid="orphans-selected-count">
          {t('selectedCreateBind', { total: totalSelected, create: createItems.length, bind: patchItems.length })}
        </span>
        <div className="spacer" />
        <button
          type="button"
        ...
        >
          {batch.status === 'submitting' ? t('applying') : t('apply')}
        </button>
      </div>
      ...
      {hasIncomplete && (
        <div className="banner err" data-testid="orphans-incomplete">
          {needsTargetIds.length > 0 && (
            <div>
              {t('missingTarget', { count: needsTargetIds.length, plural: needsTargetIds.length === 1 ? '' : 's' })}
            </div>
          )}
          {cloudTargetIds.length > 0 && (
            <div data-testid="orphans-cloud-rejected">
              {t('cloudRejected', { count: cloudTargetIds.length, plural: cloudTargetIds.length === 1 ? '' : 's' })}
            </div>
          )}
        </div>
      )}
 ...
 * the configured local runtime endpoint (no endpoint field). These are the
 * only safe bind targets for orphans — redirecting a cloud model's
 * backendModel would leave the endpoint/key cloud wiring in place and
 * silently misroute.
 */
