    ...
    return (
      <section className="panel full" data-testid="alias-conflicts-empty">
        <PageHeader
          ...
          title={t('aliases')}
          sub={t('aliasesEmptySub')}
          right={<Pill kind="ok">0 {t('conflicts')}</Pill>}
        />
        <div className="empty">
          <span className="tone-ok">●</span> {t('noAliasConflicts')}
        </div>
  ...
  return (
    <section className="panel full" data-testid="alias-conflicts-page">
      <PageHeader
        ...
        title={t('aliases')}
        sub={t('aliasesSub')}
        right={<Pill kind="warn" testId="alias-conflict-count">{conflicts.size} {conflicts.size === 1 ? t('conflict') : t('conflicts')}</Pill>}
      />
          ...
          >
            <h3 className="conflict-alias">
              ...
              <span className="tone-muted" style={{ fontSize: 12 }}>
                {t('claimedByModels', { count: models.length })}
              </span>
            </h3>
                ...
                return (
                  <li key={m.id} className="conflict-model" data-testid={`conflict-model-${alias}-${m.id}`}>
                    <div className="conflict-model-header">
                      ...
                      {m.isDefault && <span className="tone-ok" style={{ marginLeft: 8 }}>★ {t('defaultBadge')}</span>}
                    </div>
                    ...
                    <div className="conflict-preview tone-muted" data-testid={`preview-${alias}-${m.id}`}>
                      {t('aliasesAfter')} <code>{proposedAliases.length > 0 ? proposedAliases.join(', ') : t('noneParen')}</code>
                    </div>
                  </li>
                )
              })}
      ...
      <div className="batch-bar">
        <span className="tone-muted" data-testid="alias-proposed-count">
          {t('modelsWillPatch', { count: proposed.length, plural: proposed.length === 1 ? '' : 's' })}
        </span>
        ...
        >
          {batch.status === 'submitting' ? t('applying') : t('applyPlan')}
        </button>
      </div>
