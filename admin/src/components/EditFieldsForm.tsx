...
export function EditFieldsForm({ status, submitting, error, onSubmit, onCancel }: Props) {
    ...
    if (headersRaw.trim() === '') {
      ...
      try {
        ...
        if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
          setHeadersError(t('headersMustObject'))
          return
        }
        headers = parsed as Record<string, string>
      } catch {
        setHeadersError(t('invalidJson'))
        return
      }
    }
  ...
  return (
    <div className="form" data-testid="edit-fields-form">
      <div className="form-hint">{t('editHint')}</div>
      <TextField field="label" value={label} onChange={setLabel} testId="field-label" autoFocus />
      <CsvField field="aliases" values={aliases} onChange={setAliases} testId="field-aliases" />
      <TextField field="backendModel" value={backendModel} onChange={setBackendModel} testId="field-backendModel" />
      <TextField field="endpoint" value={endpoint} onChange={setEndpoint} testId="field-endpoint" placeholder="https://… (blank for router-routed)" />
      <Advanced testId="edit-advanced">
        <TextField field="role" value={role} onChange={setRole} testId="field-role" />
        <NumberField field="contextWindow" value={contextWindow} onChange={setContextWindow} testId="field-contextWindow" />
        <NumberField field="timeoutMs" value={timeoutMs} onChange={setTimeoutMs} testId="field-timeoutMs" />
        <label className="field">
          <FieldLabel field="headers" />
          <span className="field-hint tone-muted">{t('headersHint')}</span>
          <textarea
      ...
      <div className="form-actions">
        ...
        <button type="button" onClick={submit} disabled={submitting} data-testid="edit-submit">
          {submitting ? t('saving') : t('saveFields')}
        </button>
      </div>
    </div>
  )
