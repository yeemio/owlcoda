  ...
  return (
    <div className="batch-results" data-testid={testId ?? 'batch-results'}>
      <div className="batch-summary">
        <span className="tone-ok" data-testid="batch-ok-count">{t('okCount', { count: okCount })}</span>
        {failCount > 0 && (
          <>
            <span className="tone-muted"> · </span>
            <span className="tone-err" data-testid="batch-fail-count">{t('failedCount', { count: failCount })}</span>
          </>
        )}
      </div>
