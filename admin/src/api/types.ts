...
export interface ModelContextCapability {
  contextWindow: number
  source: 'configured' | 'official_known' | 'runtime_discovered' | 'fallback'
...
export interface LocalRuntimesResponse {
  schemaVersion: number
  runtimes: DiscoveredLocalRuntime[]
}
...
export interface ProviderProbeResult {
  ...
  latencyMs: number
  detail: string
  /** Provider template id the probe ran against (e.g. 'kimi'). Truthful breadcrumb so users can verify which provider was actually hit. */
  provider: string
  /** Fully-resolved URL the probe sent its request to. Empty string when the probe short-circuited before URL resolution. */
  endpoint: string
  /** Backend model id that the probe sent in the request body. */
  backendModel: string
