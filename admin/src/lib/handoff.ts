...
export function pathToRoute(path: string): HandoffRoute {
  // Old #/start deep-links redirect to models — start page is removed.
  if (path === 'start' || path.startsWith('start/')) return 'models'
  ...
  // 'runs' path retired from product UI (stale stress-test snapshot, no real
  // audit-log feed yet). Old #/runs links silently fall through to 'models'.
...
function routeToPath(route: HandoffRoute): string {
  switch (route) {
    // 'start' is kept in HandoffRoute for compat but routes to /models now.
    case 'start': return '/models'
